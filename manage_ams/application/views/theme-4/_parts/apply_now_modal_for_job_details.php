...
$primary_phone_number = isset($formpost['phone_number']) ? phonenumber_format($formpost['phone_number'], true) : '';
$is_regex = 1;
...
<script>
    ...
    var sm_enable = <?= $job_details['sms_module_status'] == 1 ? 1 : 0 ?>;

    function validate_form() {
        var sm_regex = sm_enable ? /(\(\d{3}\))\s(\d{3})-(\d{4})$/ : /^[0-9\-]+$/;
        youtube_check()
        $("#register-form").validate({
            ...
            submitHandler: function(form) {
                ...
                if (sm_enable == 1) {
                    // Double check
                    var sm_value = $('#PhoneNumber').val();
                    //
                    var match_length = 10;
                    //
                    if (sm_value.indexOf('+1') === false) {
                        match_length = 11;
                    }
                    //
                    sm_value = sm_value.replace(/[^0-9]/g, '');
                    if (sm_value.length != match_length) {
                        return alert(
                            'You must enter a valid US phone number to enable sms. e.g. +1 (XXX)-XXX-XXXX'
                        );
                    }
                }
                    ...
                    <ul>
                        <form class="popup-form" method="post" name="register-form" enctype="multipart/form-data" id="register-form">
                            ...
                            <li>
                                ...
                                <?= $input_group_start; ?>
                                <input class="form-fields js-phone" id="PhoneNumber" type="text" name="phone_number" required placeholder="Phone Number" value="<?= $primary_phone_number; ?>">
                                <?= $input_group_end; ?>
                            ...
                            <li>
                                ...
                                <select class="form-fields" name="country" onchange="getStates(this.value, '<?php echo $states; ?>')" required="required">
                                    <?php foreach ($active_countries as $active_country) { ?>
                                        <option value="<?php echo $active_country['sid']; ?>" <?php if ($active_country['sid'] == $country_id) { ?> selected <?php $country_id = $active_country['sid'];
                                                                                                                                                            }
                                                                                                                                                                ?>>
                            ...
                            <li></li>
                            <!--  -->
                            <li class="<?= $job_details['sms_module_status'] == 1 ? '' : 'hidden'; ?>">
                                ...
                                <label class="control control--radio">
                                    <input type="radio" name="contactPreference" value="email" checked /> Email
                                    <div class="control__indicator"></div>
                                </label>
                                <label class="control control--radio">
                                    <input type="radio" name="contactPreference" value="sms" /> SMS
                                    <div class="control__indicator"></div>
                                </label>
                            </li>
