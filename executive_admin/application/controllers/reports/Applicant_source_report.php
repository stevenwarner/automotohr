...
class Applicant_source_report extends CI_Controller
{
    ...
    public function index($company_sid, $keyword = 'all', $job_sid = 'all', $applicant_type = 'all', $start_date = 'all', $end_date = 'all', $page_number = 1)
    {
        if ($this->session->userdata('executive_loggedin')) {
            ...
            $total_records = $this->Reports_model->get_applicants($company_sid, $keyword, $job_sid, $applicant_type, 'all', $start_date_applied, $end_date_applied, true);

            $this->load->library('pagination');

            $pagination_base = base_url('reports/applicant_source_report') . '/' . $company_sid . '/' . urlencode($keyword) . '/' . $job_sid . '/' . urlencode($applicant_type) . '/' . urlencode($start_date) . '/' . urlencode($end_date);
            ...
            $applicants = $this->Reports_model->get_applicants($company_sid, $keyword, $job_sid, $applicant_type, 'all', $start_date_applied, $end_date_applied, false, $per_page, $offset);
            $data['applicants'] = $applicants;
            $data['applicants_count'] = $total_records;

            /** export sheet file * */
            if (isset($_POST['submit']) && $_POST['submit'] == 'Export') {
                $applicants = $this->Reports_model->get_applicants($company_sid, $keyword, $job_sid, $applicant_type, 'all', $start_date_applied, $end_date_applied, false, $total_records, 0);
                $data['applicants'] = $applicants;
            ...
            $this->load->view('reports/applicant_source_report');
