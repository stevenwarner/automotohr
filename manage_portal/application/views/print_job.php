<html>
    ...
    <body style="background: #fff !important; font-size: 11px; ">
        <link rel="StyleSheet" type="text/css" href="http://localhost/automoto/templates/AutomotoSocial Jobs/main/images/design.css">
    ...
    <body style="background: #fff !important">
        ...
        <div class="printPage">
            <div class="printLeft">
                ...
                <img src="<?php echo AWS_S3_BUCKET_URL . $company_details["Logo"]; ?>" width="150">
                    <br/><br/>
                <?php } ?>
                <span class="strong"><?=$company_details["CompanyName"]?></span>
                <?php if (!empty($company_details["address"])) { ?>
                    <br/><?=$company_details["address"]?>
                <?php } ?>
                <?php if (!empty($company_details["phone"])) { ?>
                    <br/>Phone: <?=$company_details["phone"]?>
                <?php } ?>
                <?php if (!empty($company_details["email"])) { ?>
                    <br/>Email: <?=$company_details["email"]?>
                <?php } ?>
                <?php if (!empty($company_details["server_name"])) { ?>
                    <br/><?php echo STORE_PROTOCOL.$company_details["server_name"]; ?>
                <?php } ?>
