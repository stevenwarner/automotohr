...
class Zip_recruiter_feed extends CI_Controller {
...
private function addLastRead($sid){
    ...
    public function index() {
        ...
        foreach ($jobData as $job) {
            if (in_array($job['user_sid'], $activeCompaniesArray)) {
				...
				$JobDescription ='<br><br>Job Description:<br><br>'.$job['JobDescription']; 

                if (isset($job['JobRequirements']) && $job['JobRequirements'] != NULL) {
                    $JobDescription .= '<br><br>Job Requirements:<br><br>' . strip_tags($job['JobRequirements'], '<br>');
                }
    ...
    private function isActiveFeed(){
        ...
        $validSlug = $this->all_job_feed_model->check_for_slug('ziprecruiter_paid');
        if(!$validSlug){
