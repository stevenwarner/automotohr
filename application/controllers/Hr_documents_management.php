...
//
     ...
     * 
     */
    function getSubmittedDocument($document_sid, $request_type, $request_from, $letter_request = NULL,$historyflag = NULL) {
        
        $form_input_data = "NULL";
        $is_iframe_preview = 1;
        
        if($historyflag==1){
             $document = $this->hr_documents_management_model->get_requested_authorized_content($document_sid, 'assigned_document_history');
             $requested_content = $this->hr_documents_management_model->get_requested_content($document_sid, $request_type, 'assigned_document_history', 'P&D', 1);
            $file_name = $this->hr_documents_management_model->get_document_title($document_sid, $request_type, 'assigned_document_history');
        }else{
            $document = $this->hr_documents_management_model->get_requested_authorized_content($document_sid, $request_from);
