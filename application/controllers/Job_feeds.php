    ...
    public function index($slug) {
        ...
        foreach ($jobData as $job) {
            if (in_array($job['user_sid'], $activeCompaniesArray)) {
                ...
                $companyPortal = $this->all_job_feed_model->get_portal_detail($company_id);

                if(empty($companyPortal)) {
                    continue;
                }

                $companydata = $this->all_job_feed_model->get_company_name_and_job_approval($company_id);
                ...
                $feed_data = $this->all_job_feed_model->fetch_uid_from_job_sid($uid);
                ...
                if ($JobCategorys != null) {
                    ...
                    foreach ($cat_id as $id) {
                        $job_cat_name = $this->all_job_feed_model->get_job_category_name_by_id($id);
        ...
        ';
        echo trim($rows);
        mail(TO_EMAIL_DEV, 'Feed XML: ' . date('Y-m-d H:i:s'), print_r($listedJobArray, true));
        exit;
    }
}
