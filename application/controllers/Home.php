...
class Home extends CI_Controller {
    ...
    function generate_menu_tree() {
        ...
        for($i=0; $i<count($parent_menu); $i++) {
            ...
            $main_menu[$main_sid] = array(  'main_sid'                          => $parent_menu[$i]['main_sid'],
...
//            }
            $main_menu_url[$url_code] = array('sid'                             => $parent_menu[$i]['main_sid'],
                'name'                            => $parent_menu[$i]['name'],
                'code'                            => $url_code,
                'fa_icon'                         => $fa_icon,
            ...
            if($sid > 0) {
                if($status > 0) {
                    ...
                    $sub_menu[$main_sid][] = array( 'sid'                       => $sid,
                    ...
                    $sub_menu_chain[$url_code][] = array(   'name'              => $parent_menu[$i]['title'],
                        'url_code'          => $sub_url_code,
                        'link'              => $link);

                    $sub_menu_url[$sub_url_code] = array('sid'                  => $sid,
                        'name'                 => $parent_menu[$i]['title'],
                        'type'                 => $parent_menu[$i]['type'],
        ...
        $return_array = array('main'                                            => $main_menu,
            'sub'                                             => $sub_menu,
...
//        echo '<pre>'; print_r($return_array); exit;
    ...
    function event_detail($token = null){
        ...
        if(empty($event_exists)){
            $this->load->view('errors/html/error_event_not_found');return;
        }
        // Check if not set to false
        if(MAX_EVENT_HISTORY_ENTRIES){
            $entries = $this->cm->check_event_history_limit(
