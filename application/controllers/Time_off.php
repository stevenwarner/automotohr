...
class Time_off extends Public_Controller
{
    ...
    */
    ...
    function importHistorical()
    {
        ...
        //
        $this->load->view('main/header', $data);
        $this->load->view('timeoff/importhistorical');
        $this->load->view('main/footer');
    }
    ...
    function importHistoricalTimeOff()
    {
        // Check for ajax request
        if (!$this->input->is_ajax_request()) $this->resp();
        ///
        ...
        // Check post size and action
        if (!sizeof($post)) $this->resp();

        if (!isset($post['companySid']) || $post['companySid'] == '') $this->resp();
        ...
        foreach ($post['timeoffs'] as $timeoffrow) {
            ...
            //
            if (in_array($timeoffrow['policy'], $post['approversPolicyDatacheck']['timeoffPolices'])) {
                $policy =  get_polidySid($timeoffrow['policy']);
                $insert_data['timeoff_policy_sid'] = $policy;
            }
            //
            ...
            if ($timeoffrow['status'] != '' || $timeoffrow['status'] != 0) {
                $status = $timeoffrow['status'];
                $insert_data['status'] = $status;
            }

            $insert_data['reason'] = $timeoffrow['employee_comment'];
