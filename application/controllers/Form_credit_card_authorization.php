...
class Form_credit_card_authorization extends CI_Controller {
    ...
    public function index($verification_key = null, $pre_fill_flag = null) {
        ...
        if ($verification_key != null) {
            $document_record = $this->documents_model->get_document_record('credit_card_authorization_form', $verification_key);
            ...
            if (!empty($document_record) && ($document_record['status'] != 'signed') || $pre_fill_flag == 'view') {
                ...
                if ($this->form_validation->run() == false) {                                    
                    ...
                    $this->load->view('form_credit_card_authorization/index', $data);
                } else {
                    $dataToSave = array();
                    ...
                    $this->documents_model->update_document_record('credit_card_authorization_form', $verification_key, $dataToSave, $status);

                    if ($pre_fill_flag != null && $pre_fill_flag == 'pre_fill') {
                        $this->session->set_flashdata('message', 'Form Successfully Pre-Filled.');
                        $this->documents_model->insert_document_ip_tracking_record($company_sid, 0, getUserIP(), 'credit_card_authorization_form', 'pre_filled', $_SERVER['HTTP_USER_AGENT']);
                        ...
                        $this->documents_model->insert_document_ip_tracking_record($company_sid, $employer_sid, getUserIP(), 'credit_card_authorization_form', 'signed', $_SERVER['HTTP_USER_AGENT']);
