...
class Blocked_ips extends Admin_Controller
{
    ...
    public function witelist()
    {
        ...
        check_access_permissions($security_details, $redirect_url, $function_name); // Param2: Redirect URL, Param3: Function Name
        //
        $this->data['title'] = 'White List IPs';
        $this->data['subtitle'] = 'List Of All White List IPs';
        //
        $this->render('manage_admin/block_ips/viewallowed', 'admin_master');
    }
        ...
        switch ($form_data['action']) {
            ...
            case 'save_ips';
                ...
                foreach ($form_data['ip_list'] as $k0 => $v0) {
                    ...
                    //
                    $is_exist = $this->ipmodal->check_ip_new($v0);
                    //
                    $ip_list[$k0]['status'] = 1;
                    if ($is_exist) {

                        if ($is_exist[0]['is_locked'] == 0) {
                            $this->db->query("UPDATE blocked_ips  SET is_locked = 1 WHERE ip_address = '$v0';");
                ...
                if ($new_ips_rows != '') {
                    $new_ips_rows = "\n" . $new_ips_rows;
                    blockunblockips($new_ips_rows, $v0);
                }
            ...
            case 'fetch_records_white':
                ...
                //
                $ips = $this->ipmodal->get_ips_white(
                    $inset,
                    $offset
                );
                //
            ...
            case 'save_ips_allowed';
                ...
                foreach ($form_data['ip_list'] as $k0 => $v0) {
                    ...
                    if ($is_exist) {
                        ...
                        if ($is_exist[0]['is_locked'] == 1) {
                            $this->db->query("UPDATE blocked_ips  SET is_locked = 0 WHERE ip_address = '$v0';");
                            $new_ips_rows .= "DENY FROM $v0" . "\n";
                        } else {
                            $ip_list[$k0]['status'] = 0;
                            continue;
                ...
                if ($new_ips_rows != '') {
                    $new_ips_rows = "\n" . $new_ips_rows;
                    blockunblockips($new_ips_rows, $v0, 0);
                }
