...
class Applicants_between_period_report extends Admin_Controller {
    ...
    public function index($company_sid = 'all', $start_date = 'all', $end_date = 'all', $oem = 'all', $all = 1, $keyword = 'all', $page_number = 1) {
        ...
        $function_name      = 'applicants_between_period_report';
        ...
        $this->data['page_title'] = 'Advanced Hr Reports - Applicants Between Period';
        ...
        $pagination_base = base_url('manage_admin/reports/applicants_between_period_report') . '/' . $company_sid . '/' . urlencode($start_date) . '/' . urlencode($end_date) . '/' . urlencode($oem) . '/' . urlencode($all) . '/' . urlencode($keyword);
...
//                }
            ...
            if($start_date_applied==NULL || $end_date_applied==NULL)
                ...
                $this->data['page_title'] = 'Advanced Hr Reports - Applicants Between ( ' . date('m-d-Y', strtotime($start_date_applied)) . ' - ' . date('m-d-Y', strtotime($end_date_applied)) . ' )';
        ...
        $total_records = $this->advanced_report_model->GetAllApplicantsBetween($company_sid, $oem, $start_date_applied, $end_date_applied, $keyword, 0, false, 1, $all);
        $config = array();
        ...
        //-----------------------------------Pagination Ends-----------------------------//
        $this->data['applicants'] = $this->advanced_report_model->GetAllApplicantsBetween($company_sid, $oem, $start_date_applied, $end_date_applied, $keyword, 0, false, 0, $all, $per_page, $offset);
...
//            $this->data['applicants'] = $this->advanced_report_model->GetAllApplicantsBetween($company_sid, $brand_sid, $start_date, $end_date, 0);
//            $this->data['page_title'] = 'Advanced Hr Reports - Applicants Between ( ' . date('m-d-Y', strtotime($start_date)) . ' - ' . date('m-d-Y', strtotime($end_date)) . ' )';
//        }
        ...
        $this->render('manage_admin/reports/applicants_between_period_report');
    }
