...
class New_hires_onboarding_report extends Admin_Controller {
    ...
    public function index($company_sid = 'all', $start_date = 'all', $end_date = 'all', $oem = 'all', $all = 1, $keyword = 'all', $page_number = 1){
        ...
        $function_name      = 'new_hires_onboarding_report';
        ...
        $this->data['page_title'] = 'Advanced Hr Reports - New On-Boarding Hires';
        ...
        $pagination_base = base_url('manage_admin/reports/new_hires_onboarding_report') . '/' . $company_sid . '/' . urlencode($start_date) . '/' . urlencode($end_date) . '/' . urlencode($oem) . '/' . urlencode($all) . '/' . urlencode($keyword);
        if($company_sid != 'all' || $oem != 'all'){
            ...
            if($start_date_applied!=NULL && $end_date_applied!=NULL)
                $this->data['page_title'] = 'Advanced Hr Reports - New Hires On-Boarding Between ( ' . date('m-d-Y', strtotime($start_date_applied)) . ' - ' . date('m-d-Y', strtotime($end_date_applied)) . ' )';
            else
                $this->data['page_title'] = 'Advanced Hr Reports - New On-Boarding Hires';
        }
        else{
        ...
        $total_records = $this->advanced_report_model->GetAllApplicantsOnboarding($company_sid, $oem, $start_date_applied, $end_date_applied, $keyword, true, 1, $all);
        $config = array();
        ...
        $this->data['applicants'] = $this->advanced_report_model->GetAllApplicantsOnboarding($company_sid, $oem, $start_date_applied, $end_date_applied, $keyword, true, 0, $all, $per_page, $offset);
...
//            }
//
//            $this->data['applicants'] = $this->advanced_report_model->GetAllApplicantsOnboarding($company_sid, $brand_sid, $start_date, $end_date, true);
//            $this->data['page_title'] = 'Advanced Hr Reports - New Hires On-Boarding Between ( ' . date('m-d-Y', strtotime($start_date)) . ' - ' . date('m-d-Y', strtotime($end_date)) . ' )';
//            $this->data['is_hired_report'] = true;
//            //** get new hires **//
//
//            $this->data['flag'] = true;
...
//        }
            ...
            if (isset($this->data['applicants']) && sizeof($this->data['applicants']) > 0) {
                ...
                if (isset($_GET['company_or_brand']) && $_GET['company_or_brand'] == 'brand') {
                    if (isset($this->data['is_hired_report']) && $this->data['is_hired_report'] == true) {
                        ...
                        fputcsv($output, array('Job Title', 'Applicant Name', 'Company Name'));
                    }
                } else {
                    if (isset($this->data['is_hired_report']) && $this->data['is_hired_report'] == true) {
        ...
        $this->render('manage_admin/reports/new_hires_onboarding_report');
    }
}
