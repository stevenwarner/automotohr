...
class Time_to_fill_job_report extends Admin_Controller {
    ...
    public function index($company_sid = 'all', $brand_sid = 'all', $all = 1, $page_number = 1) {
        ...
        $function_name      = 'time_to_fill_job_report';
        ...
        $pagination_base = base_url('manage_admin/reports/time_to_fill_job_report') . '/' . $company_sid . '/' . $brand_sid . '/' . urlencode($all);
        ...
        $this->data['page_title'] = 'Advanced Hr Reports - Time To Fill';
...
//            echo $company_sid . ' com ' . $brand_sid . ' bran ' . $job['sid'] . ' job ' . 1 . '<br>';
...
//        }
        ...
        if (isset($_POST['submit']) && $_POST['submit'] == 'Export') {
            if (isset($this->data['jobs']) && sizeof($this->data['jobs']) > 0) {
                ...
                if (isset($_GET['company_or_brand']) && $_GET['company_or_brand'] == 'brand') {
                    fputcsv($output, array('Job Title', 'Job Date', 'Applicants', 'Average Days To Fill', 'Company Name'));
                } else {
                ...
                foreach ($this->data['jobs'] as $job) {
                    ...
                    $input['average_days_to_fill'] = $job['average_days_to_fill'] . ' Days(s)';
                    if (isset($_GET['company_or_brand']) && $_GET['company_or_brand'] == 'brand') {
        ...
        $this->render('manage_admin/reports/time_to_fill_job_report');
    }
