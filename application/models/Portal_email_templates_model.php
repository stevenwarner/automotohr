...
class portal_email_templates_model extends CI_Model
{
    ...
    {
        $this->db->select('job_sid,desired_job_title');
        $this->db->where('sid', $sid);
        ...
        if (!empty($applicant_data)) {
            $this->db->select('Title,Location_City,Location_State');
            $this->db->where('sid', $applicant_data[0]['job_sid']);
            $portal_job_listings_data = $this->db->get('portal_job_listings')->result_array();
            if (!empty($portal_job_listings_data)) {
                return $portal_job_listings_data;
            } else {
