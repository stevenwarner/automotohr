...
class Job_screening_questionnaire_model extends CI_Model
{
    ...
    //
    function get_individual_question_details_indeed($val, $score, $questions_sid)
    {
        $this->db->select('value, score, result_status');
    ...
    //
    function insert_indeed_result($data)
    {
        $this->db->insert('candidate_jsons', $data);
    }


    //
    function getindeedJobData($company_sid, $applicant_job_list_sid)
    {
        $this->db->select('*');
        $this->db->where('sid', $applicant_job_list_sid);
        $this->db->where('company_sid', $company_sid);
        $this->db->where('applicant_source', 'https://www.indeed.com');
        $logo = $this->db->get('portal_applicant_jobs_list');
        $result = $logo->row_array();
        return $result;
    }
}
