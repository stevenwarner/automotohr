...
class Hr_documents_management_model extends CI_Model
{
    ...
    public function assignGroupDocumentsToUser(
        ...
        //
        if ($groupDocuments == true) {
            $data['session'] = $this->session->userdata('logged_in');
            $company_sid = $data['session']['company_detail']['sid'];

            $this->assigneGroupDocuments($company_sid, $userType, $userId);
        }
    ...
    //
    function assigneGroupDocuments($company_sid, $user_type, $user_sid)
    {

        $data['session'] = $this->session->userdata('logged_in');

        $eeid = $data['session']['employer_detail']['sid'];
        $jobs_listing = NULL;
