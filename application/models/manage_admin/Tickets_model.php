...
class Tickets_model extends CI_Model {
    ...
    function get_all_tickets($limit = null, $start = null, $status = '')
    {
        if($status == 'awaiting'){
            $this->db->where('status', 'Awaiting Response');
        } else if ($status == 'feedback') {
            $this->db->where('status', 'Feedback Required');
    ...
    function get_all_tickets_count($status = ''){
        if($status == 'awaiting'){
            $this->db->where('status', 'Awaiting Response');
        } else if ($status == 'feedback') {
            $this->db->where('status', 'Feedback Required');
    ...
    function add_ticket_message($insert_array, $ticket_status){
        ...
        $this->db->update('tickets', array('status' => $ticket_status, 'updated_date' => date('Y-m-d H:i:s')));
    ...
    function close_this_ticket($sid, $data_to_update){
        $this->db->where('sid', $sid);
        $this->db->update('tickets', $data_to_update);
    }
}
