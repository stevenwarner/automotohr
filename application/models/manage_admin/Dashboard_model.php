...
class Dashboard_model extends CI_Model {
    ...
    function event_detail($event_sid, $exec = FALSE){
        ...
        if($event['user_id'] != NULL){
            ...
            if((int)$event['user_id'] !== 0){
                $table = $event['user_type'] == 'affiliate' ? 'affiliations' : 
                ( $event['user_type'] == 'demo' ? 'free_demo_requests' : 
                    ( $event['user_type'] == 'admin' ? 'administrator_users' : 'client_refer_by_affiliate')
                );
                ...
                if($user_id_array != ''){
                    ...
                    foreach ($user_id_array as $k0 => $v0) {
                        switch ($v0['type']) {
                            case 'affiliate': $table = 'affiliations'; break;
                            case 'demo': $table = 'free_demo_requests'; break;
                            case 'admin':
                            case 'super admin': $table = 'administrator_users'; break;
                            default: $table = 'client_refer_by_affiliate'; break;
                        }
    ...
    function getOrganicJobCount() {
        ...
        foreach ($result_arr as $k0 => $v0) {
            ...
            if($approvalRight['has_job_approval_rights'] != 1) { $organicCount++; continue; }
