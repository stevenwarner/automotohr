...
class Dashboard_model extends CI_Model
{
    ...
    function get_events($type, $year, $month, $day, $week_start = FALSE, $week_end = FALSE, $admin_id, $event_type, $exec = FALSE)
    {
        ...
        foreach ($events as $k0 => $v0) {
            ...
            // Added on: 02-05-2019
            $events[$k0]['json']['status'] = $admin_event_history != NULL ? ($admin_event_history == 'cannotattend' ? 'cancelled' : ($admin_event_history == 'reschedule' ? 'rescheduled' : $admin_event_history
                )
            ) : $v0['event_status'];
            //
            $events[$k0]['json']['last_status']  = $admin_event_history != NULL ? ($admin_event_history == 'cannotattend' ? 'cancelled' : ($admin_event_history == 'reschedule' ? 'rescheduled' : $admin_event_history
                )
            ) : 'pending';
    ...
    {
        $this->db->where('applicant_type', 'Applicant');
        $this->db->where('date_applied between "' . (date('Y-m-01', strtotime('now'))) . '" and "' . (date('Y-m-t', strtotime('now'))) . '"', null);
        $this->db->from('portal_applicant_jobs_list');
        return $this->db->count_all_results();
    }
    ...
    {
        $this->db->where('applicant_type', 'Applicant');
        $this->db->where('date_applied between "' . (date('Y-01-01', strtotime('now'))) . '" and "' . (date('Y-12-31', strtotime('now'))) . '"', null);
        $this->db->from('portal_applicant_jobs_list');
        return $this->db->count_all_results();
    }
