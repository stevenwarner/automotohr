<?php class Incident_report_model extends CI_Model {

    function __construct() {
        parent::__construct();
    }

    function add_incident_type($data) {
        $this->db->insert('incident_type', $data);
        return $this->db->insert_id();
    }

    function get_all_incident_types() {
        $this->db->select('*');
    ...
    function get_incident_type($id) {
        $this->db->select('*');
        $this->db->where('id', $id);
        $type = $this->db->get('incident_type')->result_array();
        return $type;
    }
    ...
    function get_manager_list() {
        ...
        $this->db->where('status', '1');
        $managers = $this->db->get('incident_type')->result_array();
        return $managers;
    }

    function update_incident_type($id, $data) {
        $this->db->where('id', $id);
        $type = $this->db->update('incident_type', $data);
        return $type;
    }

    function update_incident_question($id, $data) {
        $this->db->where('id', $id);
        $type = $this->db->update('incident_questions', $data);
        return $type;
    }

    function fetch_incident_name($id) {
        $this->db->select('incident_name, safety_checklist');
        $this->db->where('id', $id);
        $name = $this->db->get('incident_type')->result_array();
        return $name;
    }

    function fetch_questions($id) {
        $this->db->select('incident_questions.*', 'incident_type.incident_name');
        $this->db->where('incident_type_id', $id);
        $this->db->join('incident_type', 'incident_questions.incident_type_id = incident_type.id', 'left');
        $questions = $this->db->get('incident_questions')->result_array();
        return $questions;
    }

    function add_new_question($data) {
        $this->db->insert('incident_questions', $data);
        return $this->db->insert_id();
    }
    ...
    function get_company_incident_name($cid, $inc_id) {
        ...
        $inc_name = $this->db->get('incident_type')->result_array()[0]['incident_name'];
