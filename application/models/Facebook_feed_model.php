...
class facebook_feed_model extends CI_Model
{
    ...
    function get_all_organic_jobs($featuredArray, $approved = 0)
    {
        $this->db->where('portal_job_listings.active', 1);
        //        $this->db->where('organic_feed', 1);
        $this->db->where_not_in('portal_job_listings.sid', $featuredArray);
        if ($approved) {
            $this->db->select('users.CompanyName,portal_job_listings.*');
            $this->db->where('portal_job_listings.approval_status', 'approved');
            $this->db->where('portal_job_listings.ppj_product_id >', 0);
            $this->db->where('users.parent_sid', 0);
            $this->db->where('users.active', 1);
            $this->db->where('users.per_job_listing_charge', 1);
            $this->db->where("expiry_date > '2016-04-20 13:26:27' OR expiry_date IS NULL");
            $this->db->join('users', 'users.sid = portal_job_listings.user_sid', 'inner');
        }
        return $this->db->get('portal_job_listings')->result_array();
    }

    function get_all_company_jobs_indeed_organic($featuredArray, $approved = 0)
    {
    ...
    function get_all_active_companies($ppj = 0)
    {
        if ($ppj) {
            $result = $this->db->query("SELECT `sid` FROM `users` WHERE `parent_sid` = '0' AND `per_job_listing_charge` = 1 AND `is_paid` = 1 AND `active` = '1' AND (`expiry_date` > '2016-04-20 13:26:27' OR `expiry_date` IS NULL)")->result_array();
        } else {
            $result = $this->db->query("SELECT 
                `sid` 
                FROM `users` 
                WHERE `parent_sid` = '0' 
                AND `career_site_listings_only` = 0 
                AND `per_job_listing_charge` = 0 
                AND `is_paid` = 1 
                AND `active` = '1' 
                AND (`expiry_date` > '2016-04-20 13:26:27' OR `expiry_date` IS NULL)")->result_array();
        }
        if (count($result) > 0) {
            $data = array();
            foreach ($result as $r) {
    ...
    function save_feed_response($data)
    {
        $this->db->insert('facebook_feed_applications', $data);
        return $this->db->insert_id();
    }
    ...
    function get_company_sid($job_sid)
    {
        $this->db->select('user_sid');
        $this->db->where('sid', $job_sid);
        $company_sid = $this->db->get('portal_job_listings')->result_array();
        if (sizeof($company_sid)) {
            return $company_sid[0]['user_sid'];
        } else {
            return 0;
        }
    }
    ...
    function getJobDetails($id)
    {
        $a = $this->db
            ...
            users.CompanyName
        ')
            ->join('users', 'portal_job_listings.user_sid = users.sid', 'inner')
            ->where('portal_job_listings.sid', $id)
    ...
    //
    function saveIncomingApplicant($in)
    {
        $this->db->insert('facebook_applicants', $in);
        return $this->db->insert_id();
    }


    //
    function updateIncomingApplicant($up, $id)
    {
        $this->db
            ->where('sid', $id)
            ->update('facebook_applicants', $up);
    }
    ...
    function alreadyApplied(
        ...
        // Set return array
        $r = [
            'portalJobApplicationsId' => 0,
            'hasApplied' => 0
        ];
        // Check if applicant exists against company
        $a = $this->db
            ->select('sid')
            ->where('employer_sid', $companyId)
        ...
        //
        if (!sizeof($b)) {
            // Let's insert the applicant
            $this->db
                ->insert('portal_job_applications', [
                    'first_name' => $firstName,
                    'last_name' => $lastName,
                    'email' => $emailAddress,
                    'phone_number' => $phoneNumber,
                    'employer_sid' => $companyId
                ]);
            //
            $r['portalJobApplicationsId'] = $this->db->insert_id();
        } else $r['portalJobApplicationsId'] = $b['sid'];
        // Check if applicant applied for a job
        $a = $this->db
            ->select('sid')
            ->where('job_sid', $jobId)
            ->where('portal_job_applications_sid', $r['portalJobApplicationsId'])
            ->get('portal_applicant_jobs_list');
        //
        $b = $a->row_array();
        $a = $a->free_result();
        //
        if (sizeof($b)) $r['hasApplied'] = 1;
        //
        return $r;
    }
