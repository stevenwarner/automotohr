...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, id, password, active, last_login, timezone')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['users']);
	...
	**/
	 ...
	 * @author aditya menon
	*/
	public function create_group($group_name = FALSE, $group_description = '', $additional_data = array(), $available_modules = array())
	{
		...
		$data = array('name'=>$group_name, 'description'=>$group_description, 'available_modules'=>$available_modules);
	...
	public function update_group($group_id = FALSE, $group_name = FALSE, $additional_data = array(), $permissions = array())
	{
		...
		// IMPORTANT!! fourth parameter was string type $permissions; this following code is to maintain backward compatibility
		// New projects should work with 4th param as array
		if (is_string($permissions)) $permissions = array('permissions' => $permissions);
		...
		if (!empty($additional_data)) $data = array_merge($this->_filter_data($this->tables['groups'], $additional_data), $data);
                ...
                // filter out any data passed that doesnt have a matching column in the groups table
		// and merge the set group data and the additional data
		if (!empty($permissions)) $data = array_merge($this->_filter_data($this->tables['groups'], $permissions), $data);
