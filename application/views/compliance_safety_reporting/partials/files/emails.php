                                    ...
                                    <?php foreach ($report['libraryItems'] as $d_key => $item) { ?>
                                        <?php if ($item['file_type'] == 'document' || $item['file_type'] == 'image' ) { ?>
                                            <tr>
                                                ...
                                                $item_url       = '';
                                                $item_category  = '';
                                                $item_type      = strtolower($extension);
                                                $item_path      = $item['s3_file_value'];
                                                $item_title     = $item['title'];

                                                if ($item_type == 'pdf') {
                                                    $item_category = 'Document';
                                                    $document_url = 'https://docs.google.com/gview?url=' . AWS_S3_BUCKET_URL . $item_path . '&embedded=true';
                                                } else if (in_array($item_type, ['doc', 'docx'])) {
                                                    $item_category = 'Document';
                                                    $document_url = 'https://view.officeapps.live.com/op/embed.aspx?src=' . urlencode(AWS_S3_BUCKET_URL . $item_path);
                                                } else if (in_array($item_type, ['jpe', 'jpg', 'jpeg', 'png', 'gif'])) {
                                                    $item_category = 'Image';
                                                    $document_url = AWS_S3_BUCKET_URL . $item_path;
                                                    $item_type    = "image";
                                                }
                                                ?>
                                        ...
                                        <?php } else { ?>
                                            <tr>
                                                ...
                                                <td class="text-center">
                                                    <label class="control control--checkbox" style="margin-left:10px; margin-top:10px;">
                                                        <input class="select_lib_item" id="med_key_m_<?php echo $item['sid']; ?>" type="checkbox" item-category="Media" item-title="<?php echo $media_title; ?>" item-type="<?php echo $media_type; ?>" item-sid="m_<?php echo $item['sid']; ?>" />
                                                        <div class="control__indicator"></div>
                                                    </label>
                                                </td>
                                            </tr>
