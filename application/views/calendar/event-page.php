...
<?php
	...
	if($event_details['address'] != '' || $event_details['address'] != NULL){
		...
		$event_info_box .= '					<img src="https://maps.googleapis.com/maps/api/staticmap?center='.(urlencode($event_details['address'])).'&zoom=13&size=250x150&key='.GOOGLE_API_KEY.'&markers=color:blue|label:|' . urlencode($event_details['address']).'" />';
		$event_info_box .= '				</a>';
		$event_info_box .= '			</div>';
		$event_info_box .= '		</div>';
		$event_info_box .= '	</div>';
