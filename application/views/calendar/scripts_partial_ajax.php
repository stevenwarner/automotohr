...
<style>
    ...
    .modal{ z-index: 1062; }
    .js-category-list{ z-index: 1000; }
    .js-timeoff-edit-request{
        cursor:pointer;
    }
    .popover{ width: 100%; }
    .popover-title{ background-color: #81b431; color: #ffffff;}
...
<?php 
    ...
    if(!check_blue_panel_status()) {
        unset($calendar_opt['event_type_info']['employee'][1],
...
<script>
    ...
    $(function () {
        ...
        $('#js-employee-select').on('select2:select', function(e){
            ...
            unselect_interviewer({ id: selected_employer_id});
        });
        ...
        $('#js-interviewers-select').on('select2:select', function(e){
            ...
            // Create email row
            select_interviewer(
                {id: obj['employer_id'], employee_type: obj['employee_type'], 'label': obj['full_name'] }
            );
        });
        ...
        $('#js-interviewers-select').on('select2:unselect', function(){
            ...
            unselect_interviewer({ id: diff[0] });
        });
        ...
        calendar_ref.fullCalendar({
            ...
            eventDragStart: function (event) {
                if(Object.size(drag_event) == 0 || event.event_id != drag_event.id){
                    drag_event = { id: event.event_id, start_time : event.start, end_time : event.end, date : event.eventdate };
                }
            },
            // Triggers when an event is drop
            eventDrop: function (event, delta, revertFunc) { update_event(event, revertFunc, 'drag'); },
            ...
            eventResize: function(event){ update_event(event, undefined, 'resize'); },
        ...
        $(document).on('click', '.js-users-type', function() {
            ...
            if($(this).val() == 'personal') { load_personal_view(); $('.js-comment-text > span').text(personal_comment_text); }
        ...
        // Remove body overlay
        // on modal close
        $(document).on('hidden.bs.modal', '#js-event-modal',  function() { $('body').removeClass('ajs-no-overflow'); });
        ...
        $(document).on('click', '.js-delete-interviewer', function() {
            ...
            $.each(interviewers_list_array, function(i,v){
                if(v == id) { interviewers_list_array.splice(i,1); return false; }
            })
        ...
        //
        $('.js-person-email-check-input').click(function() { $('.js-person-email').toggle(); });
        ...
        function form_check() {
            ...
            if(user_type == 'employee' && $('.js-selected-event-category').val() != 'training-session'){
                if($('#js-employee-select').val() == '' || $('#js-employee-select').val() == null){
                    alertify.alert('Please, select an employee', flush_alertify_cb); return false;
                }
            }
        ...
        function update_event(event, revert, type){
            ...
            if( event.expired_status === true ){
                ...
                }, flush_alertify_cb).set('labels', {ok:'Yes', cancel:'No'});
                // Revert the changes
                revert();
                return false;
            }
        ...
        function add_event(date, js_event){
            ...
            select_interviewer({id: "<?=$employer_id;?>", employee_type: get_ie_obj(<?=$employer_id;?>)['employee_type'], label: get_ie_obj(<?=$employer_id;?>)['full_name']});
            $('#js-save').show();
            func_hide_loader();
        }
        ...
        function edit_event(e){
            ...
            $.get("<?=base_url();?>calendar/get-event-detail/"+ e.event_id +"", function(resp){
                ...
                if(event.users_type == 'applicant'){
                    ...
                    load_personal_view({category: e.category});
                ...
                switch (e.category) {
                    ...
                    case 'other': event_category = 'Other'; break;
                    default: event_category = event.category_uc.replace(/-/g, ' '); break;
                }
                //
                if(is_personal_check === false || e.category == 'other'){
                    ...
                    $('.js-category-dropdown').css({'background-color' : event_color_obj[e.category], 'color': '#fff'});
                    //
                    $('#js-event-date').val(moment(event.date).format(site_date_format+" dddd"));
                    //
                    $('#js-event-start-time').val(event.event_start_time);
                ...
                if(personal_status === false || (e.category == 'training-session' || e.category == 'other')){
                    ...
                    if(event.interviewers_details.length > 0){
                        ...
                        //
                        select_interviewer( { id: "<?=$employer_id;?>", label: get_ie_obj(<?=$employer_id;?>)['full_name'], employer_type: get_ie_obj(<?=$employer_id;?>)['employer_type'] } );
                    }
                    ...
                    } else{ $('#js-reminder-email-wrap').hide(0); }
                }
        ...
        function event_process(e){
            ...
            fetch_events_by_date(calendar_OBJ.current.date_array, function(resp){
                ...
                // check for records
                if(resp.Status === false) { func_hide_loader(); return false; }
                // save latest records
        ...
        function get_applicant(req, res){
            ...
            applicantXHR = $.get("<?=base_url();?>calendar/get-applicant/"+ req.term +"", function(resp){
                if(!resp) { $('.js-search-loader-applicant').hide(0); return false; }
        ...
        function select_interviewer(item, selected_item){
            ...
            if(selected_item !== undefined){
                ...
                $.each(selected_item, function(i, v) {
                     if(v == item.id){
                        selected = 'checked="checked"'; return false;
                     }
                });
            }
        ...
        function unselect_interviewer(e){ $('.int_ems_'+e.id+'').remove(); }
            ...
            $.each(jobs, function(i, v) {
                ...
                if(selected_jobs.length > 0) {
                   $.each(selected_jobs, function(i0, v0) {
                       if(v0 == v.list_sid){ selected='checked="checked"'; return false;}
                   });
                }
                rows += '<div class="col-xs-6">';
                rows += '   <label class="control control--checkbox">';
        ...
        // Hide loader
        function func_hide_loader() { $('#my_loader, .file_loader').hide(); }

        // Show loader
        function func_show_loader() { $('#my_loader, .file_loader').show(); }
        ...
        function load_personal_view(edit_mode){
            ...
            switch(selected_category){
                ...
                case 'meeting':
                    ...
                    if($('#js-meeting-check').prop('checked') === false)
                        $('#js-meeting-box').hide(0);
                    load_categories('personal', { selected: 'meeting'});
                break;
                default:
                    reset_modal_view();
                    $('label#attendees_label').text('Assigned Attendees');
                    ...
                    if($('#js-meeting-check').prop('checked') === false)
                        $('#js-meeting-box').hide(0);
                    load_categories('personal', { selected: 'other'});
                break;
            }
        }
