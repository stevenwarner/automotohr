    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">Authorized E-Signture</h4>
            </div>
            <div class="modal-body">
                <form id="form_e_signature" enctype="multipart/form-data" method="post" action="<?= base_url() ?>onboarding/ajax_e_signature">
                    <input type="hidden" name="perform_action" value="save_e_signature" />
                    <input type="hidden" name="company_sid" value="<?php echo $company_sid; ?>" />
                    <input type="hidden" name="first_name" value="<?php echo $employer_first_name; ?>" />
                    <input type="hidden" name="last_name" value="<?php echo $employer_last_name; ?>" />
                    <input type="hidden" name="email_address" value="<?php echo $employer_email; ?>" />
                    ...
                    <div class="row">
                        <div class="col-lg-12 text-center">
                            <button onclick="common_func_save_e_signature();" type="button" class="btn btn-success break-word-text" ><?php echo SIGNATURE_CONSENT_BUTTON; ?></button>
                        </div>
                    </div>
...
<script type="text/javascript">
    ...
    function common_func_save_e_signature() {
        if ($('#form_e_signature').validate()) {
            var f_sign = true;
            var f_init =  true;
            if ($('input[name="active_signature"]:checked').val() == 'drawn') {
            ...
            if($("#e_signature_user_consent").prop('checked') != true){
                f_init = false;
                f_sign = false;
                alertify.error("Authorized Consent is required!");
            }
    ...
    function save_e_signature () {
        setTimeout(function(){
            ...
            var myurl = "<?= base_url() ?>hr_documents_management_test/authorized_e_signature";
            ...
            $.ajax({
                ...
                success: function(auth_sign_sid){
                    $('#my_loader').hide();
                    alertify.success('Authorized E_signature add Successfully');
                    $('#auth_sign_sid').val(auth_sign_sid);
                    var page_url = '<?php echo $this->uri->segment(2); ?>';
                    if (page_url == 'edit_hr_document' || page_url == 'generate_new_document') {
                        $('#form_new_document').submit();
                    } else if (page_url == 'documents_assignment') {
