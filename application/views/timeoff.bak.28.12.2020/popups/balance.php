...
<script>
    ...
    // Step 1
    async function startBalanceProcess(employeeId){
        // Set the employee id
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h3 class="modal-title">Time off Balance</h3>
                    </div>
        ...
        let policies = await fetchPolicies({
                ...
                employeeSid: employeeId !== undefined ? employeeId : balanceEmployeeId
            });
        //
        ...
        //
        $('#jsBPModalTR').html(getTimeRow(policies[0]['format']));
        //
        let policyList = {};
        //
        balanceFormat = policies[0]['format'];
        balanceTimeslot = policies[0]['employee_timeslot'];
        //
        ...
        Object.keys(policyList).map((p) => {
            ...
            policy.map((pi) => {
                //
                policyOptions += `<option ${pi.PolicyStatus.Reason != '' ? `data-reason="${pi.PolicyStatus.Reason}"` : ''}" value="${pi.sid}">${pi.title} (${ pi.TotalBreakDown !== undefined ? pi.TotalBreakDown.text : 'Unlimited'})</option>`;
            });
            //
    ...
    $(document).on('click', '.jsBalanceSaveBtn', async (e) => {
        ...
        obj.employeeSid = balanceEmployeeId;
    ...
    $(document).on('click', '.jsBalanceHistoryBtn', async () => {
        ...
        if(balanceHistory.Status === false || balanceHistory.Data.length === 0){
            $('#jsBalanceHistoryTable').html(`
                <tr>
                    <td colspan="5"><p class="text-center alert alert-info">No records found.</p></td>
                </tr>
            `);
            //
            return;
    ...
    function fetchBalanceHistory(){
        return new Promise((res, rej) => {
            ...
            $.post("<?=base_url('timeoff/handler');?>", {
                ...
                companySid: <?=$company_sid;?>,
                employeeTimeSlot: balanceTimeslot,
                employeeFormat: balanceFormat,
                employeeSid: balanceEmployeeId
            }, (resp) => {
                res(resp);
            });
        });
