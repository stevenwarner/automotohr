...
jhondoe@example.dev, Paid Time Off, No, , 01-06-2020, 8, Approved, Sons High school bowling match, Make sure to submit tickets
                            ...
                            <ul>
                                <form method="post" action="javascript:void(0)" id="js-import-form" enctype="multipart/form-data">
                                    ...
                                    <li class="form-col-50-left">
                                        <input type="submit" class="btn btn-success js-submit-btn disabled" value="Import Time Off" disabled="true" />
                                    </li>
                                </form>
                            </ul>
                        </div>
...
<script>
    ...
    $(function importCSV(){
        ...
        function in_array(index){
            ...
            for(i; i < len; i++) if(index == array[i]) return 'email_address';
            // Reset start and length
            i = 0; len = policyTitles.length; array = policyTitles;
            for(i; i < len; i++) if(index == array[i]) return 'policy';
            // Reset start and length
            i = 0; len = partialTitles.length; array = partialTitles;
            for(i; i < len; i++) if(index == array[i]) return 'partial_leave';
            // Reset start and length
            i = 0; len = partialNoteTitles.length; array = partialNoteTitles;
            for(i; i < len; i++) if(index == array[i]) return 'partial_leave_note';
            // Reset start and length
            i = 0; len = requestedDateTitles.length; array = requestedDateTitles;
            for(i; i < len; i++) if(index == array[i]) return 'requested_date';
            // Reset start and length
            i = 0; len = requestedHoursTiles.length; array = requestedHoursTiles;
            for(i; i < len; i++) if(index == array[i]) return 'requested_hours';
            // Reset start and length
            i = 0; len = requestStatusTitles.length; array = requestStatusTitles;
            for(i; i < len; i++) if(index == array[i]) return 'status';
            // Reset start and length
            i = 0; len = reasonTitles.length; array = reasonTitles;
            for(i; i < len; i++) if(index == array[i]) return 'reason';
            // Reset start and length
            i = 0; len = commentTitles.length; array = commentTitles;
            for(i; i < len; i++) if(index == array[i]) return 'comment';
            return -1;
        }
