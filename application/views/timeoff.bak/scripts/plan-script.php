    ...
    $(function(){
        ...
        function setTable(resp){
            ...
            $.each(resp.Data, function(i, v){
                ...
                rows += '                <p><a href="<?=base_url('employee_profile');?>/'+( v.employee_id )+'" target="_blank">Id: '+( getEmployeeId(v.employee_id) )+'</a></p>';
                rows += '            </div>';
                rows += '        </div>';
                rows += '    </td>';
                rows += '    <td>';
                rows += '        <div class="text">';
                rows += '            <p>'+( v.plan_title )+' Year'+( v.plan_title > 1 ? 's' : '' )+'</p>';
                rows += '        </div>';
                rows += '    </td>';
                rows += '    <td>';
                rows += '        <div class="text">';
                ...
                rows += '            <a href="javascript:void(0)" class="action-edit js-edit-plan-btn"><i class="fa fa-pencil-square-o fa-fw"></i></a>';
        ...
        $('#js-save-add-btn').click(function(e){
            ...
            if($('.js-plan-type-add[value="custom"]').prop('checked') == true){
                if($('#js-plan-period-add-input').val().trim() <= 0){
                    alertify.alert('ERROR!', 'Plan period can not be less than 0.');
                    return;
                }
                megaOBJ.plan = $('#js-plan-period-add-input').val().trim();
        ...
        $('#js-save-edit-btn').click(function(e){
            ...
            if($('.js-plan-type-edit[value="custom"]').prop('checked') == true){
                ...
                megaOBJ.plan = $('#js-plan-period-edit-input').val().trim();
        ...
        function fetchPlanList(){
            $.post(baseURI+'handler', {
                ...
                //
                plans = resp.Data;
                var tmp = plans.map(function(v){ return '<option value="'+( v.timeoff_plan_list_sid )+'">'+( ucwords(v.plan_title) )+' year'+( v.plan_title <= 1 ? '' : 's')+'</option>'; });
