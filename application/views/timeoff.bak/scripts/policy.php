    ...
    $(function(){
        ...
        $('#js-employee-add').select2MultiCheckboxes({
            templateSelection: function(selected, total) {
                $('#js-employee-count').text(selected.length);
        ...
        function setTable(resp){
            ...
            $.each(resp.Data, function(i, v){
                ...
                rows += '            <a href="javascript:void(0)" class="action-edit js-edit-row-btn"><i class="fa fa-pencil-square-o fa-fw"></i></a>';
        ...
        function showEditPage(policy){
            ...
            $('#js-employee-edit').select2MultiCheckboxes({
                templateSelection: function(selected, total) {
                    ...
                    return "Selected " +( $('#js-employee-edit').val() == 'all' ? total : selected.length )+ " of " + total;
                }
            });
        ...
        function loadPlans(data, type){
            ...
            plans.map(function(v, i){
                ...
                if(data !== undefined && data.length !== 0){
                    var record = findRecord(data, v.plan_id, 'timeoff_plan_sid');
                    if(record.allowed_timeoff != null && record.allowed_timeoff > 0 )
                    slot = formatMinutes( record.format, record.default_timeslot, record.allowed_timeoff, false, true );
                }
                rows += '<li data-id="'+( v.plan_id )+'" data-timeslot="'+( v.default_timeslot )+'" data-format="'+( v.format )+'" class="js-plan-row-'+(type)+'">';
                rows += '    <div class="timeline-content margin-right">';
                rows += '        <h4 class="timeline-field-title">'+( v.plan_title )+' Year'+( v.plan_title == 1 ? '' : '(s)' )+'</h4>';
                rows += '        <div class="row">';
        ...
        function fetchEmployees(){
            $.post(baseURI+'handler', {
                ...
                companySid: <?=$company_sid;?>
                    ...
                    $('#js-employee-add').select2MultiCheckboxes({
                        templateSelection: function(selected, total) {
                            $('#js-employee-count-add').text(selected.length);
                            return "Selected " + selected.length + " of " + total;
                        }
                    });
