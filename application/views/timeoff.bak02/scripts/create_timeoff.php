            ...
            fetchEmployeeAllPolicies(employeeSid, function(resp){
                ...
                Object.keys(policyList).map((p) => {
                    ...
                    rows += '<optgroup label="'+(p)+'">';
                        policy.map((pi) => {
                            rows += `<option ${pi.PolicyStatus.Reason ? `data-reason="${pi.PolicyStatus.Reason}" disabled` : ''} value="${pi.sid}">${pi.title} (${pi.timeoff_breakdown !== undefined ? pi.timeoff_breakdown.text : 'Unlimited'})</option>`;
                        });
                    rows += '</optgroup>';
                });
