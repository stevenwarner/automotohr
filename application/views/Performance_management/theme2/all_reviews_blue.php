<?php
    ...
    if(!empty($AssignedReviews)){
        ...
        foreach($AssignedReviews as $review){
            ...
            $tr .= '            </br> ';
            $tr .= '            Due In: '.($review['end_date'] < $now ? 'Past Date' : dateDifferenceInDays($now, $review['end_date'], '%a')).' Day(s)';
            $tr .= '        </p>';
            $tr .= '    </td>';
            $tr .= '    <td>';
            $tr .= '        <p class="csF16 csB7">';
            $tr .= '            <a href="'.( purl("review/{$review['sid']}/{$review['reviewee_sid']}/{$review['reviewer_sid']}")).'" class="btn btn-orange csF14">Start Review</a>';
            $tr .= '        </p>';
            $tr .= '</tr>';
            //
