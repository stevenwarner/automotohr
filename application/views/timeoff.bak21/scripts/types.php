<script type="text/javascript">
    ...
    $(function(){
        ...
        $('#js-save-add-btn').click(function(e){
            ...
            //
            if(megaOBJ.category == ''){
                alertify.alert('ERROR!', 'Type name is required.', function(){ return ;});
                return;
            }
            //
            loader('show');
        ...
        function loadEditPage(record){
            ...
            //
            var tmp = policies.map(function(v){ return '<option value="'+( v.sid )+'">'+( v.title )+' ('+( v.is_archived == 1 ? 'Archived' : 'Active' )+')</option>'; });
            //
        ...
        // Page events
        // Page handlers
        $("#js-add-type-btn").click(function() { setHistory('add', baseURI+'types/add', 'Add Page'); });
        $(".js-view-type-btn").click(function() { setHistory('view', baseURI+'types/view', 'View Page'); });
        // Pages
        ...
        function fetchTypeCreators(){
            $.post(baseURI+'handler', {
                ...
                //
                employees = resp.Data;
                var tmp = employees.map(function(v){ return '<option value="'+( v.employee_id )+'">'+( remakeEmployeeName(v) )+'</option>'; });
        ...
        function fetchTypesList(){
            $.post(baseURI+'handler', {
                ...
                //
                types = resp.Data;
                var tmp = types.map(function(v){ return '<option value="'+( v.type_id )+'">'+( v.type_name )+'</option>'; });
...
<style>.js-archive-type > i{ color: #a94442 !important; }</style>
<style>.js-activate-type > i{ color: #81b431  !important; }</style>