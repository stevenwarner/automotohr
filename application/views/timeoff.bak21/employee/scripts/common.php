<script>
    ...
    $(document).on('change', '#js-types', function(){
        ...
        policies.map(function(policy){
            if(policy.Category == v){
                rows += `<option ${policy.PolicyStatus.Reason ? `data-reason="${policy.PolicyStatus.Reason}" disabled` : ''} value="${policy.sid}">${policy.title} (${policy.timeoff_breakdown !== undefined ? policy.timeoff_breakdown.text : 'Unlimited'})</option>`;
            }
        });
        ...
        $('#js-policies').select2({
            templateResult: (item) => {
                ...
                return `<div  ${ $(item.element).data('reason') !== undefined ? 'class="csSelect2CError bg-danger"' : ''} >${item.text} ${ $(item.element).data('reason') !== undefined ? ` <i class="fa fa-question-circle jsPopover" data-title="Why?" data-content="${$(item.element).data('reason')}"></i>` : ''}</div>`;
            },
            escapeMarkup: (i) => {
                return i;
            }
    ...
    $(document).on('select2:open', '#js-policies', function(){
        //
        $('.jsPopover').popover({
            html: true,
            trigger: 'hover'
