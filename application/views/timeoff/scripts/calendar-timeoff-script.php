...
<script type="text/javascript">
    ...
    $(function(){
        ...
        $(document).on('click', '#js-edit-modal-save-btn', function(){
            var megaOBJ = {};
            megaOBJ.sendEmail = $('input[name="js-send-email"]:checked').val();
            ...
            // Let's save the TO
            $.post(
                baseURI+'calendar_timeoff_handler',
                megaOBJ,
                function(resp){
                    //
        ...
        function loadEditPage(e){
            ...
            megaOBJ.employeeSid = <?=$employerData['sid'];?>;
            //
            $.post(baseURI+'calendar_timeoff_handler', megaOBJ, function(resp) {
                //
                if(resp.Status === false){
        ...
        function startEditProcess(resp){
            ...
            //
            //emt.status.find('option[value="approved"]').remove();
            //emt.status.find('option[value="cancelled"]').remove();
            console.log(resp.Data.Info.status);
            //if(resp.Data.Info.status == 'approved') emt.status.prepend('<option value="approved">Approved</option>');
            emt.status.val(resp.Data.Info.status);
            emt.status.select2();
            fetchEmployeeAllPolicies(resp.Data.Info.employee_sid, function(res){
                ...
                if(resp.Data.Info.img.length == 0)
                    $('#js-eme-img').prop('src', "<?=base_url('/');?>assets/images/img-applicant.jpg");
                else
                    $('#js-eme-img').prop('src', "<?=AWS_S3_BUCKET_URL;?>"+(resp.Data.Info.img)+"");
                $('#js-eme-name').text(resp.Data.Info.full_name);
                $('#js-eme-id').text(resp.Data.Info.employee_sid);
                ...
                //
                if(resp.Data.Info.status == 'cancelled' || resp.Data.Info.is_editable == false) { 
                    $('#js-edit-modal-save-btn').hide(0); 
                    emt.modal.find('input, textarea, select').prop('disabled', true); 
                    //emt.status.prepend('<option value="cancelled">Cancelled</option>');
                    $(".send-email-wrapper").hide();
                    $(".js-edit-modal-comment-wrapper").hide();
                }
                else { 
                    $('#js-edit-modal-save-btn').show(0); 
                    emt.modal.find('input, textarea, select').prop('disabled', false); 
                    $(".send-email-wrapper").show();
                    $(".js-edit-modal-comment-wrapper").show();
                }
                emt.status.select2('val', resp.Data.Info.status);
                // Show modal
                emt.modal.modal();   
                loader('hide');             
            });
        ...
        $(document).on('click', '.js-timeoff-edit-request', loadEditPageFormCalendar); 
        function loadEditPageFormCalendar(e){
            ...
            var megaOBJ = {};
            megaOBJ.action = 'get_single_request';
            megaOBJ.requestSid = $(this).attr('timeoff-request-id');
            megaOBJ.companySid = <?=$company_sid;?>;
            megaOBJ.employeeSid = <?=$employerData['sid'];?>;
            //
            $.post(baseURI+'calendar_timeoff_handler', megaOBJ, function(resp) {
