...
<script type="text/javascript">
    ...
    $(function() {
        var
            ...
            isManager = "<?= isset($_SESSION['TeamMembers']) && $session['employer_detail']['access_level_plus'] == 0 && $session['employer_detail']['pay_plan_flag'] == 0 ? true : false; ?>"
        employees = [],
            employeePolicies = [],
            departments = [],
            is_filter = false,
            ...
            filterOBJ = {
                ...
                policySid: 'all',
                requesterSid: 'all',
                startDate: $('#js-filter-from-date').val() != '' ? moment($('#js-filter-from-date').val(), 'MM-DD-YYYY').format('YYYY-MM-DD') : '',
                endDate: $('#js-filter-to-date').val() != '' ? moment($('#js-filter-to-date').val(), 'MM-DD-YYYY').format('YYYY-MM-DD') : '',
                sortType: 'upcoming',
                status: 'pending'
            },
            record = [],
        ...
        //
        var members = <?= isset($_SESSION['TeamMembers']) ? json_encode($_SESSION['TeamMembers']) : "false"; ?>;
        ...
        $(document).on('click', '#js-edit-modal-save-btn', function() {
            ...
            if (megaOBJ.isPartial === 1) {
                if (megaOBJ.startTime == '') {
                    alertify.alert('WARNING!', 'Start time of partial is required.', function() {
                        return;
                    });
                    return;
                }
                if (megaOBJ.endTime == '') {
                    alertify.alert('WARNING!', 'End time of partial is required.', function() {
                        return;
                    });
                    return;
                }
        ...
        $(document).on('click', '.js-request-title-li', function(e) {
            ...
            //
            selectedCategory = $(this).data('key');
            //
        ...
        function applyFilter(e) {
            ...
            filterOBJ.sortType = $('.js-sort-select').val();

            fetchRequests();
        }

        $('.js-sort-select').change(applyFilter);
        // Fetch plans
        function fetchRequests() {
            ...
            xhr = $.post(baseURI + 'handler', filterOBJ, function(resp) {
                ...
                if (resp.Status === false) {
                    ...
                    $('.type_and_filter_nav').hide();
                    return;
                }
                //
                $('.type_and_filter_nav').show();
                //
                setTable(resp.Data);
                //
                drawPendingChart(pa);
            ...
            if (requests.Requests.length == 0) {
                ...
                for (i; i < l; i++) {
                    ...
                    if (ea[it] !== undefined) {
                        ...
                        // if (requests['Requests'][i]['archive'] != 1) {
                        ea[it] = {};
                    ...
                    $.each(leave_schedule_data, function(i, e) {
                        leave_schedule_detail += 'Date : ' + (moment(e.date, 'MM-DD-YYYY').format(timeoffDateFormat)) + '<br> Type : ' + (e.partial == 'partialday' ? 'Partial Day' : 'Full Day') + '<br> Duration : ' + (e.breakdown.text) + '<br /><br />';
                    });
                    //
                    // Date: 19/10/2020
                    // note: The below line comment because we allow archive PTO request to list
                    // if (requests['Requests'][i]['archive'] == 1) {
                    //     rows += '<tr data-type="archive" data-id="'+( requests['Requests'][i]['requestId'] )+'" style="display: none;">';
                    // } else if(requests['Requests'][i]['Category'] != null){
                    ...
                    rows += getStatusHTML(requests['Requests'][i][requests['Requests'][i]['level_at'] == '3' ? 'status' : 'level_status']);
                    rows += '      </div>';
                    rows += '    </td>';
                    rows += '    <td align="center">';
                    rows += '       <div class="notes-employee">';
                    rows += '           <span class="js-edit-request icon_blue" data-toggle="tooltip" title="Edit Request" >';
                ...
                // Date: 19/10/2020
                // note: The below line comment because we shift archive tab with other "pending, rejected, cancelled, approved" tabs
                // trows += '<li class=""><a href="javascript:void(0)" data-key="archive" class="js-request-title-li">Archive</a></li>';
            }
            //
            $('#js-data-load-area').html(rows);
            $('.js-request-titles').html(trows);
            if (requests.Titles.length != 0 && requests.Titles[0] != null)
                $('#js-data-load-area tr[data-type="' + (requests.Titles[0].toLowerCase().replace(/\s+/g, '_')) + '"]').show();
            //
            $('[data-toggle="popover"]').popover({
                html: true
            });
            // Take back to last category
            if (selectedCategory != null) {
                $('.js-request-title-li[data-key="' + (selectedCategory.toLowerCase()) + '"]').click();
            }
            loader('hide');
        }
        //
        ...
        function startEditProcess(resp) {
            ...
            // emt.reason.val(resp.Data.Info.reason);
            ...
            alertify.confirm(
                ...
                function() {
                    ...
                    $.post(
                        ...
                        function(resp) {
                            alertify.alert('SUCCESS!', resp.Response, function() {
                                _this.parent().find('#js-cancel-btn').click();
                                selectedCategory = currentRequest.Info.Category;
                                fetchRequests();
                                // window.location.reload();
                            });
                        }
            ...
            $('#js-history-data-area').pagination({
                ...
                callback: function(hs, pagination) {
                    ...
                    hs.map(function(h, i) {
                        ...
                        if (employeeData['full_name'] === undefined) return '';
                        rows += `
                            <div class="row cs-timeoff-history-row">
                                <div class="col-sm-12" ${ i % 2 === 0 ? 'style="background-color: #f1f1f1; padding: 10px;"' : ''}>
                                    <div class="col-sm-3 cs-row">
                                        <h5><b>Employee</b></h5>
                                        <span>${ employeeData['full_name']}</span>
                                    </div>
                                    <div class="col-sm-3 cs-row">
                                        <h5><b>Role</b></h5>
                                        <span>${ overwrite[employeeData['role']]}</span>
                                    </div>
                                    <div class="col-sm-3 cs-row">
                                        <h5><b>Action</b></h5>
                                        <span>${ h.status.ucwords()} </span>
                                    </div>
                                    <div class="col-sm-3 cs-row">
                                        <h5><b>Created On</b></h5>
                                        <span>${ moment(employeeData['created_at']).format('MM/DD/YYYY')} </span>
                                    </div>
                                    ${ h['reason'] != '' && h['reason'] != null ? `
                                        <div class="col-sm-12 cs-row">
                                            <h5><b>Comment</b></h5>
                                            <span>${ h['reason'] }</span>
                                        </div>
                                    ` : '' }
        ...
        function getPolicy(sid) {
            ...
            //
            for (i; i < l; i++)
                if (employeePolicies[i]['sid'] == sid) return employeePolicies[i];
            return null;
        }
        //
        ...
        function getRequestedDaysEdit() {
            ...
            $('#js-timeoff-date-box-edit tbody tr').map(function(i, v) {
                ...
                if (time.requestedMinutes <= 0) {
                    err = true;
                    alertify.alert('WARNING!', 'Please, add request time for date <b>' + ($(this).data('date')) + '</b>.', function() {
                        return;
                    });
                } else if (time.requestedMinutes > time.defaultTimeslotMinutes) {
                    err = true;
                    alertify.alert('WARNING!', 'Requested time off can not be greater than shift time.', function() {
                        return;
                    });
                }
                //
        ...
        //
        $('.request_report_status').on('click', function() {
            $(".nav_report_status>li.active").removeClass("active");
            $(this).parent().addClass('active active-tab');
            filterOBJ.status = $(this).attr("data-key");
            //
            $('#js-filter-status').select2('val', filterOBJ.status);

            if (filterOBJ.status == 'rejected' || filterOBJ.status == 'cancelled') {
                filterOBJ.sortType = 'created_desc';
                $('.js-sort-select option[value="created_desc"]').prop('selected', true);
            } else {
                filterOBJ.sortType = 'upcoming';
                $('.js-sort-select option[value="upcoming"]').prop('selected', true);
            }
