<script type="text/javascript">
    ...
    $(function(){
        ...
        function resetFilter(e){
            e.preventDefault();
            is_filter = false;
            $('#js-filter-departments').select2('val', 'all');
            $('#js-filter-teams').select2('val', 'all');
        ...
        function fetchCompanyApprovers(){
            ...
            megaOBJ.teamSid = is_filter ? $('#js-filter-teams').val() : '';
        ...
        function setTable(resp){
            var title = fetchStatus != 'active' ? 'Activate Approver' : 'Deactivate Approver',
        ...
        $('#js-save-add-btn').click(function(e){
            ...
            megaOBJ.companySid = <?=$company_sid;?>;
            megaOBJ.employeeSid = <?=$employer_sid;?>;
            megaOBJ.isDepartment =  $('input[name="js-is-department-add"]:checked').val();
            megaOBJ.approverSid = $('#js-employee-add').val();
            megaOBJ.action = 'add_company_approver';
            megaOBJ.status = 1;
            megaOBJ.percentageCheck = Number($('#js-approve-100-percent-add').prop('checked'));
            megaOBJ.archiveCheck = Number($('#js-archive-check-add').prop('checked'));
            megaOBJ.departmentSid = $(`#js-${megaOBJ.isDepartment == 1 ? 'departments' : 'teams'}-add`).val();
            ...
            //
            if(megaOBJ.isDepartment === undefined){
                alertify.alert('ERROR!', 'Please, select type.');
                return;
            }

            // Check if title is empty
            if(megaOBJ.isDepartment == 1 && (megaOBJ.departmentSid == '' || megaOBJ.departmentSid == 0)){
                alertify.alert('ERROR!', 'Please select a department.');
                return;
            }
            if(megaOBJ.isDepartment == 0 && (megaOBJ.departmentSid == '' || megaOBJ.departmentSid == 0)){
                alertify.alert('ERROR!', 'Please select a team.');
                return;
            }
        ...
        $('#js-save-edit-btn').click(function(e){
            ...
            megaOBJ.companySid = <?=$company_sid;?>;
            megaOBJ.employeeSid = <?=$employer_sid;?>;
            megaOBJ.isDepartment =  $('input[name="js-is-department-edit"]:checked').val();
            megaOBJ.departmentSid = $(`#js-${megaOBJ.isDepartment == 1 ? 'departments' : 'teams'}-edit`).val();
            megaOBJ.approverSid = $('#js-employee-edit').val();
            megaOBJ.sid = $('#js-id-edit').val();
            megaOBJ.action = 'edit_company_approver';
            megaOBJ.status = 1;
            megaOBJ.percentageCheck = Number($('#js-approve-100-percent-edit').prop('checked'));
        ...
        function showEditPage(approver){
            ...
            //
            $(`input[name="js-is-department-edit"][value="${approver.is_department}"]`).prop('checked', true).trigger('click');
            //
            if(approver.is_department == 1){
                //
                $('#js-departments-edit').select2('val', approver.department_id == null ? 'all' : approver.department_id.split(','));
            } else{
                //
                $('#js-teams-edit').select2('val', approver.department_id == null ? 'all' : approver.department_id.split(','));
            }
            //
            // $('#js-status-edit').select2('val', approver.status);
            //
            $('#js-approve-100-percent-edit').prop('checked', Boolean(Number(approver.approver_percentage)));
        ...
        function setDepartments(){
            ...
            $('#js-departments-add').select2({ closeOnSelect: false });
            $('#js-departments-edit').html(rows);
            $('#js-departments-edit').select2({ closeOnSelect: false });
            $('#js-filter-departments').html(rows);
            $('#js-filter-departments').select2({ closeOnSelect: false });
        }

        //
        function setTeams(){
            var rows = '';
            rows += '<option value="all">All Teams</option>';

            if(departments.length != 0){
                departments.map(function(d){
                    if (d.department_id != 'all') {
                        rows += '<optgroup label="'+( d.title )+'">';
                        //
                        if(teams.length != 0){
                            teams.map(function(t){
                                if (t.department_sid == d.department_id) {
                                    rows += '<option value="'+( t.team_id )+'">'+( t.title )+'</option>';
                                }
                            });
                        }
                        //
            ...
            teams.push({ team_sid: 'all', name: 'All Teams' });
            $('#js-teams-add').html(rows);
            $('#js-teams-add').select2({ closeOnSelect: false });
            $('#js-teams-edit').html(rows);
            $('#js-teams-edit').select2({ closeOnSelect: false });
            $('#js-filter-teams').html(rows);
            $('#js-filter-teams').select2({ closeOnSelect: false });
        }
