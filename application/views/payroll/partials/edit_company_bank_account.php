...
<div class="container">
    <div class="row">
        <div class="col-sm-4">
            ...
            <input type="text" id="jsACAccountTitle" class="form-control" value="<?=$bank_account['account_title'];?>"/>
        </div>
        <div class="col-sm-4">
            <label class="csF16 csB7">Account Number <span class="csRequired"></span></label>
            <input type="text" id="jsACAccountNumber" class="form-control" value="<?=$bank_account['account_number'];?>"/>
        </div>
        <div class="col-sm-4">
            <label class="csF16 csB7">Routing Number <span class="csRequired"></span></label>
            <input type="text" id="jsACRoutingNumber" class="form-control" value="<?=$bank_account['routing_number'];?>"/>
        </div>
    </div>
    <br>
    <!--  -->
    ...
    <div class="col-sm-4">
            ...
            <input type="text" id="jsACBankName" class="form-control" value="<?=$bank_account['bank_name'];?>"/>
        </div>
        <div class="col-sm-4">
            <label class="csF16 csB7">Account Type <span class="csRequired"></span></label>
            <select id="jsACAccountType" class="form-control">
                <option <?=$bank_account['account_type'] == 'checking' ? 'selected' : '';?> value="checking">Checking</option>
                <option <?=$bank_account['account_type'] == 'savings' ? 'selected' : '';?> value="savings">Saving</option>
            </select>
        </div>
        <div class="col-sm-4">
            <br>
            <label class="control control--checkbox">
                <input type="checkbox" id="jsACUseForParoll" <?=$bank_account['use_for_payroll'] ? 'checked' : '';?>> Use For Payroll 
    ...
    <div class="row">
        <div class="col-sm-12">
            <span class="pull-right">
                <button class="btn btn-success jsACAddAccount">Update Bank Account</button>
            </span>
        </div>
    </div>
</div>
...
<script>
    $(function(){
        ...
        $('.jsACAddAccount').click(function(event){
            ...
            //
            ml(true, 'jsEBEModalLoader');
            //
            $.post(
                "<?=base_url("payroll/edit_company_payroll_bank_account")?>", 
                o
            ).done(function(resp){
                //
