    ...
    $(function(){
        ...
        calendar_ref.fullCalendar({
            ...
            select: function (start, end) {
                ...
                if (!title) { calendar_ref.fullCalendar('unselect'); return false; }
                calendar_ref.fullCalendar('renderEvent', { title: title, start: start, end: end }, true); // stick? = true
            },
            ...
            eventDragStart: function (event) {
                if(Object.size(drag_event) == 0 || event.event_id != drag_event.id){
                    drag_event = { id: event.event_sid, start_time : event.start, end_time : event.end, date : event.event_date };
                }
            },
            // Triggers when an event is drop
            eventDrop: function (event, delta, revertFunc) { update_event(event, revertFunc); },
            eventResizeStart: function(event){ if(Object.size(drag_event) == 0 || event.event_id != drag_event.id) drag_event = { start_time : event.start, end_time : event.end, date : event.eventdate }; },
            // Triggers when an event is resized
            eventResize: function(event){ update_event(event); },
        ...
        $(document).on('click', '.js-users-type', function() {
            ...
            if($(this).val() == 'personal') { load_personal_view(); }
            else if($(this).val() == 'demo') { load_demo_view(); }
        ...
        $(document).on('click', '.js-btn-category', function(){
            ...
            $('.js-category-dropdown').css({'background-color' : event_color_obj[$(this).data('id')], 'color': '#ffffff'});
        });
        ...
        $(document).on('click', '.js-btn-category-p', function(){
           // Check if last selected type is same
            ...
            $('.js-category-dropdown-p').css({'background-color' : event_color_obj[$(this).data('id')], 'color': '#ffffff'});
        ...
        $(document).on('click', '.js-demo-category-btn', function(){
           // Check if last selected type is same
            ...
            $('.js-demo-category-dropdown').css({'background-color' : event_color_obj[$(this).data('id')], 'color': '#ffffff'});
        });
        ...
        $(document).on('click', '.btn_remove_user', function () { $($(this).closest('.external_users').get()).remove(); });
        ...
        // Remove external user
        $(document).on('click', '.btn_remove_admin', function () { $($(this).closest('.external_admin').get()).remove(); });
        ...
        //
        $('#js-participants-select').on('select2-selecting', function(e) { $('.js_ps_row_'+e.val+'').show(); });
        //
        $('#js-participants-select').on('select2-removing', function(e) { $('.js_ps_row_'+e.val+'').hide(); });
        ...
        //
        $('#js-event-modal').on('hidden.bs.modal', function() { $('body').removeClass('ajs-no-overflow'); last_selected_category = null; });
        
        // Flush local data on modal close
        $(document).on('hidden.bs.modal', '#js-event-modal',  function() { $('#js-event-modal').find('input, textarea, select, button').prop('disabled', false); current_edit_event_details = {}; });
        ...
        $(document).on('click', '#js-cancel', function(e) {
            ...
            }, flush_cb).set('labels', { ok:'Yes', cancel:'No'});
        });
        ...
        $(document).on('click', '.js-reminder-email-btn', function(e) {
            ...
            if(reminder_emails.length === 0){
                alertify.alert("Error! Please, select atleast one email to send reminder.", flush_cb); return false;
            }
            //
            loader('show');
        ...
        $('.js-person-send-notification-input').click(function(event) { $('.js-person-email').toggle(); });
        ...
        function event_process(e){
            ...
            fetch_events_by_date(calendar_OBJ.current.date_array, function(resp){
                ...
                // check for records
                if(resp.Status === false) { loader(false); return false; }
                // save latest records
        ...
        function load_categories(type, target_OBJ){
            ...
            $((target_OBJ === undefined ? target_OBJ : target_OBJ.cd) === undefined ? '.js-category-dropdown' : target_OBJ.cd).css({'background-color' : event_color_obj[selected], 'color': '#ffffff'});
            target.html(rows);
        }
        ...
        function event_validation(action){
            ...
            if(include_participants){
                ...
                if(participants == null) { participants = []; participants.push(<?=$admin_id;?>); participants_show_email.push(<?=$admin_id;?>); }
                else{
        ...
        function process_event(obj, type){
            ...
            event_XHR = $.post("<?=base_url('manage_admin/process-event');?>", obj, function(resp) {
                ...
                alertify.alert(resp.Status === false ? 'Error!' : 'Success', resp.Response, flush_cb); return;
                event_XHR = null;
                loader(false);
            });
        }
        ...
        function load_edit_view(e){
            ...
            if(e.comment != '' && e.comment != null) { $('#js-comment-msg').val(e.comment); $('#js-comment-msg-box').show(); $('#js-comment-check').prop('checked', true); }
            // Check and set the reminder
            if(e.reminder_check != '0') { $('#js-reminder-select').val(e.reminder_duration); $('#js-reminder-box').show(); $('#js-reminder-check').prop('checked', true); }
        ...
        function get_user_details(user_id){
            ...
            $.each(user_list, function(i, v) {
                if(v.id == user_id) { details = v; return v; }
            });
            return details;
        }
        ...
        function get_difference_of_event(new_data){
            ...
            if(new_data.participants != '' && new_data.participants !== undefined){
                ...
                var tmp_users = [];
                _.filter(current_edit_event_details.participants, function(o){ tmp_users.push(o.id); });
                //
        ...
        function clean_external_participants(external_participants){
            ...
            for(k0 in external_participants){
                ...
                if(external_participants[k0].hasOwnProperty('email_address')){ external_participants[k0]['email'] = external_participants[k0]['email_address']; delete external_participants[k0]['email_address']; }
        ...
        function update_event(event, revert){
            if(event.is_recur == 1) { revert(); return true; }
            // updates
        ...
        function find_admin(admin_id){
            ...
            var tmp = [];
            $.each(admin_list, function(i, v) {
                if(v.admin_id == admin_id) { tmp = v; return false; }
            });
            return tmp;
        }
        ...
        function convert_to_single_array(ar, ind){
            ...
            var return_array = [];
            $.each(ar, function(i, v) { return_array.push(v[ind]); });
            return return_array;
        }
    })
</script>
