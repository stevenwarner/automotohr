...
<div class="js-ddbox">
    <div class="form-wrp">
        ...
        <div class="row" style="margin-bottom: 10px;">
            ...
            <div class="col-sm-12">
        <p class="text-justify text-danger"><stron>Note: Describes how the payment will be split. If Deposit Type is Percentage, then the split amounts must add up to exactly 100. If Deposit Type is Amount, then the last split amount must be nil to capture the remainder.</stron></p>
            </div></div>
                        ...
                        <div class="row">
                            <div class="col-sm-5">
                                ...
                                if (isset($account['deposit_type']) && $account['deposit_type'] == 'percentage') {
                                    ...
                                    $ADDON = '%';
                                } elseif (isset($account['deposit_type']) && $account['deposit_type'] == 'amount') {
                                ...
                                ?>
                                <p><strong><span id="DTT_<?php echo $i; ?>" class="js-DTT"><?php echo $DTL; ?></span> <span class="cs-required">*</span></strong></p>
                            </div>
                            <div class="col-sm-7">
    ...
    $(function() {
        $('.js-deposit_type').change(function(e) {
            ...
            if ($(this).val() == "percentage") {
                ...
                $(".js-DTT").text("Percentage value to be deposited to this account:");
                $('.js-account-percentage-addon').text('%')
            } else if ($(this).val() == "amount") {
                // $("#DTT_" + id).text("Dollar amount to be deposited to this account:");
                $(".js-DTT").text("Dollar amount to be deposited to this account:");

                $('.js-account-percentage-addon').text('$')
            } else if ($(this).val() == 0) {
                // $("#DTT_" + id).text("Percentage or dollar amount to be deposited to this account:");
                $(".js-DTT").text("Percentage or dollar amount to be deposited to this account:");

                $('.js-account-percentage-addon').text('$')
            }

            $('.js-deposit_type').val($(this).val());
        ...
        function getData() {
            ...
            $('.js-dd-row').map(function(el, i) {
                ...
                if (deposit_type == 'percentage') {
                    totalAccountPercentage += parseInt(account_percentage);
                    if (totalAccountPercentage != 100 && parseInt(el) > 0) {
                        alertify.alert('Total percentage value of account detail 1 and account detail 2 must up to exactly 100.').set({
                            title: "WARNING !"
                        });
                ...
                if (deposit_type == 'amount') {
                    ...
                    if (account_percentage != '' && account_percentage != 0) {
                        amountFound++;
                    }

                    // 
                    if (parseInt(amountFound) == 0 && parseInt(el) > 0) {

                        alertify.alert('Please provide amount atleast in one account ').set({
                            title: "WARNING !"
                        });
                    ...
                    if (parseInt(amountFound) > 1) {
                        alertify.alert('Amount must be in account detail 1 or account detail 2 not in both').set({
                            title: "WARNING !"
                        });
