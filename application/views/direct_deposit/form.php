...
<script>
    $(function() {
                            ...
                            success: function(data) {
                                if (form_data.length == (i + 1)) {
                                    if (user_type === "employee") {
                                        return pushTheDirectDepositToGusto(user_sid);
                                    }
                                    $('#my_loader').hide();
        ...
        /**
         * Send the direct deposit to payroll
         * 
         * @param {number} userId
         */
        function pushTheDirectDepositToGusto(userId) {
            $.ajax({
                url: '<?= base_url('direct_deposit') ?>/' + userId + "/push",
                cache: false,
                type: 'post',
                success: function() {
                    $('#my_loader').hide();
