        ...
        <div class="col-sm-3">
            <ul class="list-group">
                <li class="list-group-item">
                    <img src="<?=AWS_S3_BUCKET_URL.$this->session->userdata('govt_user_login')['picture'];?>" style="display: block; margin: auto; max-width: 100%"/>
                </li>
                <li class="list-group-item">
                    <p>
                        <strong>Agency Name:</strong> <br /> <?=$this->session->userdata('govt_user_login')['agency_name'];?>
                    </p>
                </li>
                <li class="list-group-item">
                    <p>
                        <strong>Agent Name:</strong> <br /> <?=$this->session->userdata('govt_user_login')['agent_name'];?>
                    </p>
                </li>
                <li class="list-group-item">
                    <a class="btn btn-success form-control" href="<?= base_url("govt_user/logout") ?>" style="margin-bottom: 30px;">Logout</a>
                </li>
            </ul>
        </div>
        <div class="col-sm-9">
                    ...
                    <?php if(!empty($users)) {?>
                        ...
                        <?php $user['full_name'] = ucwords(strtolower($user['full_name']));?>
                            <tr data-id="<?=$user['user_sid'];?>" data-type="<?=$user['form_type'];?>">
                                ...
                                <td align="center">
                                    <a href="javascript:void(0)" class="btn btn-info btn-view" file_name="<?=$user['form_type'] != 'assigned' ? $user['s3_filename'] : '';?>" data-path="<?=$user['form_type'] == 'assigned' ? '' : ''.AWS_S3_BUCKET_URL.$user['s3_filename'].'';?>" title="Display I9 form"><i class="fa fa-file-o"></i></a>
                                </td>
                                <td  align="center">
                                    <?php
                                    if(sizeof($user['documents'])){
                                        foreach ($user['documents'] as $document) {
                                            echo  '<button type="button" class="btn btn-info js-document"
                                            data-name="'.( $document['document_name'] ).'"
                                            data-path="'.( AWS_S3_BUCKET_URL.$document['s3_filename'] ).'"
...
<!-- Modal -->
<div class="modal fade" id="documentformmodal" tabindex="-1">
    <div class="modal-dialog" role="document" style="width: 90%;">
        <div class="modal-content">
             <div class="modal-header">
                  <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="documentTitle">Document</h4>
             </div>
             <div class="modal-body" id="documentformcontent" style="background:#fff">
             </div>
             <div id="js_document_modal_footer" class="modal-footer">
             </div>
        </div>
    </div>
</div>
...
<div id="my_loader" class="text-center my_loader js-loader">
    ...
    <div class="loader-icon-box">
        ...
        <div class="loader-text" style="display:block; margin-top: 35px;">
            Please wait while we are loading employees
        </div>
    </div>
</div>
...
<script>
    ...
    $(function(){
        ...
        $('.js-document').click(function(){
            var file_name = $(this).data("path");
            $('#documentTitle').text($(this).data('name'));
            $('#js_document_modal_footer').html('');
            if(file_name != ''){
                if(file_name.toLowerCase().match(/png|jpg|jpeg/)){
                    $('#documentformcontent').html('<img src="'+( $(this).data('path') )+'" style="width: 100%;" />');
                } else{
                    $('#documentformcontent').html('<iframe src="'+( $(this).data('path') )+'" style="width: 100%; height: 600px;"></iframe>');
                    var document_download_url="https://docs.google.com/gview?url="+file_name;
                    var footer_print_url ="https://docs.google.com/gview?url="+file_name;
                    ...
                    $('#js_document_modal_footer').html(footer_content);
                    $('#js_document_modal_footer').append(footer_print_btn);
                }
            }
        ...
        $(document).on('click', ".btn-view", function () {
            ...
            $('#document_modal_footer').html('');
            if(file_name != ''){
                var document_download_url="https://docs.google.com/gview?url=<?=AWS_S3_BUCKET_URL;?>"+file_name;
                var footer_print_url ="https://docs.google.com/gview?url=<?=AWS_S3_BUCKET_URL;?>"+file_name;
                footer_content = '<a target="_blank" class="btn btn-success" href="'+document_download_url+'">Download</a>';
                footer_print_btn = '<a target="_blank" class="btn btn-success" href="'+footer_print_url+'" >Print</a>';
    ...
    $(".i9form-control-input").click(function(){
        $('#checkall').prop('checked', $(".i9form-control-input:checked").length == $(".i9form-control-input").length ? true : false);
    });
