<?php 
	//
	$ats_phonenumber = trim($employer['PhoneNumber']);
	...
	//
	$ats_fullname = trim($employer['first_name'].' '.$employer['last_name']);
	$ats_sid      = trim($employer['sid']);
	//
	$ats_email_address = trim($employer['email']);
...
<div class="row">
	...
	<div class="col-sm-4 col-sm-offset-4 js-sms-view-phone">
		...
		<h4>Employee phone number is invalid. Please, update the phone number first.</h4>
		...
		<?php } else { ?>
			<h4>Employee phone number & email address is invalid. To proceed update the email address.</h4>
		<?php } ?>
	</div>
	<?php } else { ?>
...
<script>
	$(function(){
		...
		sms_img = '<img src="<?=base_url('assets/images/new_msg.gif');?>"  width="20"/>';
		//
		...
		//
		var megaOBJ = {};
		megaOBJ.id  = megaOBJ.applicant_id = <?=$employer['sid'];?>;
		...
		<?php if((int)phonenumber_validate($ats_phonenumber_e164) === 0){ ?>
			...
			//
			$('.js-sms-phone').keyup(replace_phone);
			$('.js-sms-phone').focus(replace_phone);

			function replace_phone() {
		...
		$('#js-sms-form').submit(function(e) {
			...
			inner_loader();
			//
			$.post("<?=base_url('employee_management/handler');?>", megaOBJ, function(resp) {
				if(resp.Status === false){
					alertify.alert('Error!', resp.Response);
					return;
				}
	    ...
	    function fetch_sms(){
	    	...
	    	xhr = $.post("<?=base_url('employee_management/handler');?>",{
	    		action: 'fetch_sms_employee',
	    		type: 'employee',
	    		module: 'emp',
	    		id: "<?=$employer['sid'];?>",
