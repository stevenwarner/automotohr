                                            ...
                                            <?php           foreach ($all_jobs as $job) {
                                                                if ($jobs_approval_module_status == '1') {
                                                                    if ($job['approval_status'] == 'approved') { ?>
                                                                        <option <?php echo ($job['active'] == 1) ? 'class="ats_search_filter_active"' : 'class="ats_search_filter_inactive"';?> value="<?= $job["sid"] ?>" <?php if (in_array($job["sid"], $job_sid_array)) { ?> selected="selected" <?php } ?>>
                                                                                <?php echo $job['Title']
                                                                                    . ' [';
                                                                                    echo ($job['active'] == 1) ? 'Active' : 'Inactive';
                                                                                    echo '] ';


                                                                            if($job['active'] == 1) {
                                                                                if($job['activation_date'] != NULL || $job['activation_date'] != ''){
                                                                                    echo ' - ('.( reset_datetime(array( 'datetime' => $job['activation_date'], '_this' => $this)) ).')';
                                                                                    // echo ' - ('.DateTime::createFromFormat('Y-m-d H:i:s', $job['activation_date'])->format('m-d-y h:i a') .')';
                                                                                }
                                                                            } else {
                                                                                if($job['deactivation_date'] != NULL || $job['deactivation_date'] != ''){
                                                                                    echo ' - ('.( reset_datetime(array( 'datetime' => $job['deactivation_date'], '_this' => $this)) ).')';
                                                                                    // echo ' - ('.DateTime::createFromFormat('Y-m-d H:i:s', $job['deactivation_date'])->format('m-d-y h:i a') .')';
                                                                                }
                                                                            }

                                                                            echo ' ('.common_get_job_applicants_count($job['sid'], $archived).')'; ?>
                                                                        </option>
                                            <?php                   } ?>
                                            <?php               } else { ?>
                                                                    <option <?php echo ($job["active"] == 1) ? 'class="ats_search_filter_active"' : 'class="ats_search_filter_inactive"';?> value="<?= $job["sid"] ?>" <?php if (in_array($job["sid"], $job_sid_array)) { ?> selected="selected" <?php } ?>>
                                        ...
                                        <div class="box-head">
                                            <div class="row date-bar">
                                                ...
                                                <div class="col-lg-7 col-md-7 col-xs-7 col-sm-7">
                                                    <time class="date-applied"><?=reset_datetime(array( 'datetime' => $employer_job["date_applied"], '_this' => $this));?></time>
                                                </div>
                                                <div class="col-lg-4 col-md-4 col-xs-4 col-sm-4">
                                                    ...
                                                    <!--  -->
                                                    <a href="javascript:void(0);" onclick="show_resume_popup(this);" class="pull-right aplicant-documents" data-preview-url="<?php echo $employer_job["resume_direct_link"]; ?>" data-download-url="<?php echo $employer_job["resume_download_link"]; ?>" data-file-name="<?php echo $employer_job['resume']; ?>" data-document-title="<?php echo 'Resume For '.$employer_job['job_title']; ?>" ><i class="fa fa-file-text-o"></i><span class="btn-tooltip">Resume</span></a>
                                            ...
                                            <div class="candidate-status applicat-status-edit">
                                                <div class="label-wrapper-outer">
                                                    <?php if ($have_status == false) { ?>
                                                        ...
                                                        <?php } ?>
                                                    <?php } else { ?>
                                                        <div <?php echo $employer_job['status_type'] == 'custom' ? ' style= "background-color: '.$employer_job['bar_bgcolor'].'"' : '' ?> class="selected <?php echo (isset($employer_job['status_css_class'])) ? $employer_job['status_css_class'] : ''; ?>">
                                                    ...
                                                    <div class="lable-wrapper">
                                                        ...
                                                        <?php } else { ?>
                                                            <?php foreach ($company_statuses as $status) { ?>
                                                                <div <?php echo $status['status_type'] == 'custom' ? ' style= "background-color: '.$status['bar_bgcolor'].'"' : '' ?> data-status_sid="<?php echo $status['sid']; ?>" data-status_class="<?php echo $status['css_class']; ?>" data-status_name="<?php echo $status['name']; ?>" class="label applicant <?php echo $status['css_class']; ?>">
...
//                data = JSON.parse(data);
    ...
    function show_resume_popup (source) {
        ...
        if (document_preview_url != '') {
            ...
            alertify.alert('<div class="text-center"><b>No Resume Found!</b?</div>');
        }
    }
