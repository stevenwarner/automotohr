...
class Twilioapp{
	...
	function setMessageService($message_service_type){
		$this->message_service = $this->getMessageServiceSidByType($message_service_type);
		return $this;
	}
	 ...
	 *
	 * @return Array|String
	 * Message sid will be send in case of success
	...
	function createMessageService(){
		...
		$callback_url = base_url('sms/callback/'.($this->message_service_name_code).'');
		// Create connection
		$this->connect();
	 ...
	 */
	function debug(){
		_e(get_object_vars($this), true, true);
	}
