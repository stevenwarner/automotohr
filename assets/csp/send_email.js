$(function Overview() {
        ...
        if (attachment_size > 0 && flag == 0) {
            ...
            $('#attachment_listing_data > .manual_upload_items').each(function(key) {
                ...
                if (item_status == 'pending') {
                    ...
                    var save_attachment_url = baseURL + 'compliance_safety_reporting/save_email_manual_attachment';
                    ...
                    form_data.append('company_sid', companyId);
                    form_data.append('uploaded_by', employeeId);
    ...
    $(".jsViewLibraryItem").on('click', function() {
        //
    ...
    $(".jsMarkAsRead").on('click', function() {
        var emailId = $(this).data('email_id');
        //
        mark_read(emailId);
    });
    ...
    function sendNormalEmail (type) {
        ...
        //
        $.ajax({
            url: baseURL + 'compliance_safety_reporting/send_compliance_report_email',
            cache: false,
            contentType: false,
            processData: false,
            type: 'post',
    ...
    function mark_read(email_sid) {
        var update_url = baseURL + 'compliance_safety_reporting/update_email_read_flag';
    ...
    $('#save_attach_item').on('click', function() {
        ...
        if (source == 'vimeo') {
            if ($('#attach_social_video').val() != '') {
                var myurl = baseURL   + "compliance_safety_reporting/validate_vimeo";
                $.ajax({
                    type: "POST",
                    url: myurl,
                    data: {
