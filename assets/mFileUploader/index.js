 ...
 *  })
 * 
 *  $('#fileInputReference').mFileUploader('get'); // It will return the uploaded file object
...
(function($) {
            ...
            <div class="jsMainUploadAreaWrapper">
                <div class="csUploadArea" id="${options.mainDivName}">
                    <div class="csUploadInnerArea">
                        <p>${options.text}</p>
                        ${options.fileLimit != -1 ? `<em style="color:rgb(255, 155, 0);"><i class="fa fa-warning"></i> Maximum allowed file size is ${options.fileLimit}</em><br />` : ''}
        ...
        const getFileExtension = (filename) => {
            ...
            let t = filename === undefined ? oFile.name.split('.') : filename.split('.');
        ...
        const getLimit = () => {
            //
            let sp = options.fileLimit.replace(/\s+/, '').toUpperCase().split(/([0-9]+)([A-Za-z]+)/);
            sp = [sp[1], sp[2]]; 
            //
        ...
        const readURL = (input) => {
            if (input) {
                ...
                reader.onload = (e) => {
                    $(`#${options['jsMFUPreviewFile']}`).data('key', e.target.result);
        ...
        const validateFile = (e) => {
            ...
            $(`#${options.mainImageViewer}`).parent().find('label span').text('');
        ...
        $(document).on('click',  `#${options['jsMFUClearFile']}`, function(e){
            ...
            if(options.placeholderImage != ''){
                $(`#${options['jsMFUPreviewFile']}`).data('key', options['placeholderImage']);
        ...
        $(document).on('click',  `#${options['jsMFUPreviewFile']}`, function(e){
            e.preventDefault();
            e.stopPropagation();
            //
            let 
            ...
            //
            if(tag.match(/img/) !== null) $(`#${options['jsMFUModal']} .modal-body`).html(tag);
            else{
    ...
    //
    $.fn.mFileUploader.__proto__.instances = instances;
})(jQuery || $);