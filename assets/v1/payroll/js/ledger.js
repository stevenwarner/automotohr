...
$(function importLedger() {
	...
	function formHandler(e) {
		...
		if (
			!format_index.includes("transactiondate") &&
			!format_index.includes("transactiontime") &&
			!format_index.includes("paydate")
		) {
		...
		fileData.map(function (v, rowNo) {
			...
			if (len > 2) {
				...
				if (!isValidDate(record.start_period)) {
					dateFlag = false;
					errorArray.push(
						"The start period date is not valid for row " +
							(rowNo + 1) +
							"."
					);
				}
				...
				if (!isValidDate(record.end_period)) {
					dateFlag = false;
					errorArray.push(
						"The end period date is not valid for row " +
							(rowNo + 1) +
							"."
					);
				}
				...
				if (!isValidDate(record.transaction_date)) {
					dateFlag = false;
					errorArray.push(
						"The transaction date is not valid for row " +
							(rowNo + 1) +
							"."
					);
				}
				//
				if (record.debit === undefined && record.credit === undefined) {
					amountFlag = false;
					errorArray.push(
						"The debit or credit amount is missing for row " +
							(rowNo + 1) +
							"."
					);
				} else {
                    ...
                    if (
                        ...
                        !isValidNumber(record.debit)
                    ) {
                        amountFlag = false;
                        errorArray.push(
                            "The debit is not a valid amount for row " +
                                (rowNo + 1) +
                                "."
                        );
                    }
                    if (
                        ...
                        !isValidNumber(record.credit)
                    ) {
                        amountFlag = false;
                        errorArray.push(
                            "The credit is not a valid amount for row " +
                                (rowNo + 1) +
                                "."
                        );
                    }
				...
				// 	amountFlag = false;
				// 	errorArray.push(
				// 		"The debit or credit amount must be greater then zero for row " +
				// 			(rowNo + 1) +
				// 			"."
				// 	);
				// }
				...
				} else if (!employeeFirstNameFlag && !employeeLastNameFlag) {
					if (dateFlag && amountFlag) {
						records.push(record);
					}
				} else {
	...
	function in_array(index) {
		...
		len = debit.length;
		array = debit;
		for (i; i < len; i++) if (index == array[i].trim()) return "debit";
		// Reset start and length
		i = 0;
		len = credit.length;
		array = credit;
		for (i; i < len; i++) if (index == array[i].trim()) return "credit";
		// Reset start and length
		i = 0;
		...
		array = endPeriod;
		for (i; i < len; i++) if (index == array[i].trim()) return "end_period";
		// Reset start and length
		i = 0;
		...
		len = grossPay.length;
		array = grossPay;
		for (i; i < len; i++) if (index == array[i].trim()) return "gross_pay";
		// Reset start and length
		i = 0;
		len = netPay.length;
		array = netPay;
		for (i; i < len; i++) if (index == array[i].trim()) return "net_pay";
		// Reset start and length
		i = 0;
		len = taxes.length;
		array = taxes;
		for (i; i < len; i++) if (index == array[i].trim()) return "taxes";
	...
	function startAddProcess(chunk) {
		...
		if (chunkOBJ.records[chunkOBJ.current] === undefined) {
			...
			alertify.alert(
				"SUCCESS!",
				"You have successfully imported  payroll ledger.<br>Inserted records: <b>" +
					insertedCount +
		...
		var messageRow = "Please, be patient as we import payrolls.<br />";
			...
			function (resp) {
				...
				if (chunkOBJ.current >= chunkOBJ.records.length) {
					...
					alertify.alert(
						...
						function () {
							window.location =
								baseURI + "payrolls/ledger/import";
						}
					);
					failedCount =
						existedCount =
