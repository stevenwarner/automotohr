...
$(function importCoursesCSV() {
	...
	function formHandler(e) {
		...
		var format_index = fileData[0]
			.toLowerCase()
			.replace(/[^a-z]/g, "")
			.trim();
		//
		if (
			!format_index.includes("coursetitle") &&
			!format_index.includes("title")
		) {
			alertify.alert("Not a valid format Debit Amount");
			return false;
		}
		if (
			!format_index.includes("lessonstatus") &&
			!format_index.includes("progress")
		) {
		...
		if (
			!format_index.includes("coursestatus") &&
			!format_index.includes("status")
		) {
		...
		if (
			!format_index.includes("coursetaken") &&
			!format_index.includes("attemptcount")
		) {
			alertify.alert("Not a valid format Start Period is missing");
			return false;
		}
		...
		if (
			...
			!format_index.includes("startperiod") &&
			!format_index.includes("startduration")
		) {
		...
		if (
			!format_index.includes("enddate") &&
			!format_index.includes("endperiod") &&
			!format_index.includes("endduration")
		) {
		...
		fileData.map(function (v, rowNo) {
			...
			if (len > 2) {
				...
				if (!isValidDate(record.start_date)) {
					dateFlag = false;
					errorArray.push(
						"The course start date is not valid for row " +
							(rowNo + 1) +
							"."
					);
				}
				...
				if (!isValidDate(record.end_date)) {
					dateFlag = false;
					errorArray.push(
						"The course end date is not valid for row " +
							(rowNo + 1) +
							"."
					);
				}
				...
				if (!isValidNumber(record.count)) {
					countFlag = false;
					errorArray.push(
						"The course attempted count is not valid for row " +
							(rowNo + 1) +
							"."
					);
				}
				...
				} else if (!courseTitleFlag) {
					errorArray.push(
						"The course title is missing for row " +
							(rowNo + 1) +
							"."
					);
				} else if (!courseTypeFlag) {
					errorArray.push(
						"The course type is missing for row " +
							(rowNo + 1) +
							"."
					);
				} else {
					errorArray.push(
						"The employee identity is missing for row " +
							(rowNo + 1) +
							"."
					);
				}
	...
	function in_array(index) {
		...
		len = title.length;
		array = title;
		for (i; i < len; i++) if (index == array[i].trim()) return "title";
		// Reset start and length
		i = 0;
		len = type.length;
		array = type;
		for (i; i < len; i++) if (index == array[i].trim()) return "type";
		// Reset start and length
		i = 0;
		len = progress.length;
		array = progress;
		for (i; i < len; i++) if (index == array[i].trim()) return "progress";
		// Reset start and length
		i = 0;
		len = status.length;
		array = status;
		...
		len = count.length;
		array = count;
		for (i; i < len; i++) if (index == array[i].trim()) return "count";
		// Reset start and length
		i = 0;
		len = startPeriod.length;
		array = startPeriod;
		for (i; i < len; i++) if (index == array[i].trim()) return "start_date";
		// Reset start and length
		i = 0;
		len = endPeriod.length;
		array = endPeriod;
		for (i; i < len; i++) if (index == array[i].trim()) return "end_date";
	...
	function startAddProcess(chunk) {
		...
		if (chunkOBJ.records[chunkOBJ.current] === undefined) {
			...
			alertify.alert(
				"SUCCESS!",
				"You have successfully imported  courses.<br>Inserted records: <b>" +
					insertedCount +
					"</b><br>Duplicated records: <b>" +
					existedCount +
					"</b> <br> Failed records: <b>" +
					failedCount +
					"</b>"
			);
			failedCount =
				...
				chunkOBJ.loaded =
				chunkOBJ.current =
				chunkOBJ.totalChunks =
				chunkOBJ.recordsLength =
					0;
			return;
		}
		//
		chunkOBJ.loaded += chunk.length;
		var messageRow = "Please, be patient as we import courses.<br />";
		...
		xhr = $.post(
			baseURI + "lms/courses/import_course_csv_handler",
			{
				action: "add_courses",
				courses: chunk,
			},
			function (resp) {
				...
				if (chunkOBJ.current >= chunkOBJ.records.length) {
					...
					alertify.alert(
						...
						function () {
							window.location =
								baseURI + "lms/courses/import_course_csv";
						}
					);
					failedCount =
						existedCount =
	...
	function showCourseModel() {
		...
		Model(
			...
			function () {
				...
				let trs = "";
				$.get(
					baseURI + "lms/courses/company_courses_list",
					function (resp) {
						companyCoursesList = resp.company_courses;
						...
						recordsHolder.map(function (v, i) {
							trs +=
								'<tr class="jsCourseRow" data-index="' +
								i +
								'" >';
							trs += "<td>" + v.title + "</td>";
							trs +=
								'<td><select name="companycourses[]" class="jscompanycourses invoice-fields"><option value="">Please Select</option>' +
								options +
								"</select></td>";
							trs += "</tr>";
						});
						let importbtn =
							'<br><div class="row"><div class="col-sm-9"></div><div class="col-sm-3 text-right"><button class="btn btn-success jsCourseImportBTN">Import</button></div></div>';
						...
						ml(false, "jsCheckCourseTitleLoader");
					}
				);
			}
		);
	...
	$(document).on("click", ".jsCourseImportBTN", function (event) {
		event.preventDefault();

		$(".jsCourseRow").map(function () {
			recordsHolder[$(this).data("index")].course_sid = $(this)
	...
	function addToChunks() {
		...
		//
		chunkOBJ.totalChunks = Math.ceil(
			chunkOBJ.recordsLength /
				Math.ceil(chunkOBJ.recordsLength / chunkOBJ.chunkSize)
		);
		//
