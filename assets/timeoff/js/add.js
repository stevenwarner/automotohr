$(function() {
    ...
    $(document).on('click', '.jsCreateRequest', function(e) {
        ...
        selectedEmployeeName = employeeName;
        //
        if ($(this).data('policyid') !== undefined) selectedPolicy = $(this).data('policyid');
        //
        Modal({
            Id: 'addModal',
            ...
            Body: '',
            Buttons: [
                '<button class="btn btn-orange btn-theme jsCreateTimeOffBTN">Create</button>'
            ],
    ...
    async function loadAdminProcess() {
        ...
        if (resp.Status == false || resp.Data.length == 0) {
            alertify.alert('WARNING!', 'We are unable to find any active employees.', () => {});
            return;
        }
        //
        ...
        Modal({
            ...
            `,
            Buttons: [
                '<button class="btn btn-orange dn jsCreateTimeOffBTN">Create</button>'
            ],
    ...
    async function loadAddSectionPage(loader, modalId) {
        ...
        // Get modal body
        const resp = await getEmployeePolicies(selectedEmployeeId);
        const policies = resp.Data.Policies;
        $(`#${modalId}`).find('.csModalBody .alert').remove();
        //
        if (policies.length === 0) {
            $('.jsAddTimeOff').remove();
            $(`#${modalId}`).find('.csModalBody').append(`<div class="alert alert-success text-center">We are unable to find policies against this employee.</div>`);
            ml(false, loader);
            return;
        }
        //
        ...
        $('#jsAddPolicy').select2();
        //
        if (selectedPolicy != 0) $('#jsAddPolicy').select2('val', selectedPolicy);
        //
