            ...
            function setTable(resp) {
                ...
                $(".jsCommentsPopover").popover({
                    ...
                    allComments[$(this).closest('.jsBox').data('id')][0].map(function(li) {
                        rows += `<li><strong>${li.msg}</strong> <br /> ${li.employeeName} ${li.employeeRole} <br /> ${li.time}</li>`;
                    });
                    //
                    rows += '</ul>';
    ...
    function getRequestBox(v, userRow) {
        ...
        if (v.archive == 1) {
            ...
            if (v.status == 'pending') {
                ...
                rows += `            <div class="col-sm-12">`;
                rows += `                <button class="btn alert-orange btn-theme form-control"><i class="fa fa-eye"></i>View</button>`;
                rows += `            </div>`;
            }
        }
        rows += `        </div>`;
