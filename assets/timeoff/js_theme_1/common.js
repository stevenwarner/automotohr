...
$('.jsCalendarView').click(function(e) {
    ...
    Modal({
        Id: 'calendarModal',
        Title: 'Calendar',
        Body: `<iframe src="${baseURL}calendar/my_events/iframe${window.location.pathname.match(/(lms)|(employee_management_system)|(dashboard)/gi) !== null ? '/ems' : ''}" width="100%" height="${$(window).height() - 90}"></iframe>`,
...
function getAccrualPlans(
    ...
    $('.js-plan-row-' + (type) + '').map(function(i) {
        ...
        //
        if ((l.accrualType == 0 || l.accrualType == '') && (l.accrualRate == 0 || l.accrualRate == '') && (l.accrualTypeM == 0 || l.accrualTypeM == '')) return;
        //
        ...
        //
        r.push(type == 'edit-reset' || same !== undefined ? { plan_title: l.accrualType, accrual_rate: l.accrualRate, accrual_type_m: l.accrualTypeM } : l);
    });
    //
...
function remakeRangeRows(
    ...
    for (i; i <= il; i++) {
        ...
        if ($.inArray(sd.format('MM-DD-YYYY'), holidayDates) === -1 && $.inArray(sd.format('dddd').toString().toLowerCase(), policyDayOffs) === -1) {
            ...
            rows += '                Full Day';
            rows += '                <input type="radio" name="' + (i) + '_day_type' + (slug.replace(/-/g, '_')) + '" checked="true" value="fullday" />';
            rows += '                <span class="control__indicator"></span>';
            rows += '            </label> <br />';
            rows += '            <label class="control control--radio">';
            rows += '                Partial Day';
            rows += '                <input type="radio" name="' + (i) + '_day_type' + (slug.replace(/-/g, '_')) + '" value="partialday" />';
            rows += '                <span class="control__indicator"></span>';
            rows += '            </label>';
            rows += '        </div>';
            rows += '    </th>';
            ...
            rows += setTimeView('#row_' + (i) + '', '-el' + slug + i, sd.format('MMDDYYYY'), 0);
            rows += '        </div>';
            rows += '    </th>';
            rows += '</tr>';
        }
...
function setTimeView(target, prefix, position) {
    ...
    let row = '';
    //
    if (typeof(position) === 'object') position = position.date.replace(/-/gi, '');
    //
    ...
    if (slug == 'D:H:M') {
        ...
        row += '        <input type="text" data-ids="' + (position) + '" data-page="' + (prefix.match(/edit/gi) !== null ? 'edit' : 'add') + '" data-type="hour" class="form-control js-number" id="js-request-days' + (prefix === undefined ? '' : prefix) + '"  />';
        row += '    </div>';
        row += '</div>';
        row += '<div class="col-sm-4">';
        row += '    <div class="form-group">';
        row += '        <label>Hours </label>';
        row += '        <input type="text" data-ids="' + (position) + '" data-page="' + (prefix.match(/edit/gi) !== null ? 'edit' : 'add') + '" data-type="hour" class="form-control js-number" id="js-request-hours' + (prefix === undefined ? '' : prefix) + '" value="' + (getTime(position, 'hour', prefix)) + '" />';
        row += '    </div>';
        row += '</div>';
        row += '<div class="col-sm-4">';
        row += '    <div class="form-group">';
        row += '        <label>Minutes </label>';
        row += '        <input type="text" data-ids="' + (position) + '" data-page="' + (prefix.match(/edit/gi) !== null ? 'edit' : 'add') + '" data-type="minute" class="form-control js-number" id="js-request-minutes' + (prefix === undefined ? '' : prefix) + '" value="' + (getTime(position, 'minute', prefix)) + '" />';
        row += '    </div>';
        row += '</div>';
    } else if (slug == 'D:H') {
        ...
        row += '    <div class="form-group">';
        row += '        <label>Days </label>';
        row += '        <input type="text" data-ids="' + (position) + '" data-page="' + (prefix.match(/edit/gi) !== null ? 'edit' : 'add') + '" data-type="day" class="form-control js-number" id="js-request-days' + (prefix === undefined ? '' : prefix) + '" value="' + (getTime(position, 'day', prefix)) + '" />';
        row += '    </div>';
        row += '</div>';
        row += '<div class="col-sm-6">';
        row += '    <div class="form-group">';
...
$(document).on('keyup', '.js-number', function() {
    //
    if ($(this).data('page') == 'add') timeRowsOBJ[$(this).data('ids')][$(this).data('type')] = $(this).val();
    else timeRowsOBJEdit[$(this).data('ids')][$(this).data('type')] = $(this).val();
});
...
function getTime(ind, indd, prefix) {
    ...
    if (prefix.match(/edit/gi) !== null) {
        if (!timeRowsOBJEdit.hasOwnProperty(ind)) {
            timeRowsOBJEdit[ind] = {};
            timeRowsOBJEdit[ind]['hour'] = window.timeoff.cPolicies[0]['Settings']['ShiftHours'];
            timeRowsOBJEdit[ind]['minute'] = window.timeoff.cPolicies[0]['Settings']['ShiftMinutes'];
        }
        return timeRowsOBJEdit[ind][indd];
    } else {
        if (!timeRowsOBJ.hasOwnProperty(ind)) {
            timeRowsOBJ[ind] = {};
            timeRowsOBJ[ind]['hour'] = window.timeoff.cPolicies[0]['Settings']['ShiftHours'];
            timeRowsOBJ[ind]['minute'] = window.timeoff.cPolicies[0]['Settings']['ShiftMinutes'];
        }
...
function getTimeInMinutes(typo) {
    ...
    if ($('#js-request-days' + (typo) + '').length !== 0) {
        format += 'D,';
        days = isNaN(parseInt($('#js-request-days' + (typo) + '').val().trim())) ? 0 : parseInt($('#js-request-days' + (typo) + '').val().trim());
    }
    if ($('#js-request-hours' + (typo) + '').length !== 0) {
        format += 'H,';
        hours = isNaN(parseInt($('#js-request-hours' + (typo) + '').val().trim())) ? 0 : parseInt($('#js-request-hours' + (typo) + '').val().trim());
    }
    if ($('#js-request-minutes' + (typo) + '').length !== 0) {
        format += 'M,';
        minutes = isNaN(parseInt($('#js-request-minutes' + (typo) + '').val().trim())) ? 0 : parseInt($('#js-request-minutes' + (typo) + '').val().trim());
    }
    //
    ...
    return {
        ...
        defaultTimeslotMinutes: (window.timeoff.cPolicies[0]['Settings']['Shift'] * 60),
        format: format.replace(/,$/, ''),
        formated: inText,
        requestedMinutes: (days * window.timeoff.cPolicies[0]['Settings']['Shift'] * 60) + (hours * 60) + minutes
    };
}
...
function get_array_from_minutes(fromMinutes, employeeShiftTime, format) {
    ...
    returnArray['D:H:M'] = {};
    //
    returnArray['D:H:M']['days'] = parseInt(((fromMinutes) / (employeeShiftTime * 60)));
    returnArray['D:H:M']['hours'] = parseInt((((fromMinutes) % (employeeShiftTime * 60)) / 60));
    returnArray['D:H:M']['minutes'] = parseInt((((fromMinutes) % (employeeShiftTime * 60)) % 60));
    ...
    returnArray['D'] = {};
    returnArray['D']['days'] = (fromMinutes / (employeeShiftTime * 60)).toFixed(2);
    ...
    if (returnArray[format]['days'] !== undefined) returnArray['text'] += returnArray[format]['days'] +
        ' day' + (returnArray[format]['days'] > 1 || returnArray[format]['days'] == 0 ? 's' : '') +
        ' & ';
    if (returnArray[format]['hours'] !== undefined) returnArray['text'] += returnArray[format]['hours'] +
        ' hour' + (returnArray[format]['hours'] > 1 || returnArray[format]['hours'] == 0 ? 's' : '') +
        ' & ';
    if (returnArray[format]['minutes'] !== undefined) returnArray['text'] += returnArray[format]['minutes'] +
        ' minute' + (returnArray[format]['minutes'] > 1 || returnArray[format]['minutes'] == 0 ? 's' : '') +
        ' & ';
    ...
    employeesOnOffToday.Data.map((emp) => {
        ...
        <li>
            <a 
                ...
                data-content="<span>${emp.first_name} ${emp.last_name}<br /><span>${remakeEmployeeName(emp, false)}</span></span>"
            >
