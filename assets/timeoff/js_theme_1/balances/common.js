...
function getIndex(
    ...
    if ($.inArray(index, ['employeeemail', 'email', 'emailladdress']) !== -1) return 'email';
    if ($.inArray(index, ['hours', 'consumed', 'consumedhours']) !== -1) return 'hours';
    if ($.inArray(index, ['policy', 'title', 'policytitle']) !== -1) return 'policy';
    //
    return false;
}