$(function () {
    ...
    function setTable(resp) {
        ...
        if (resp.Data.length == 0) {
            $('#js-data-area').html(`<tr><td colspan="6"><p class="alert alert-info text-center">No time-offs found.</p></td></tr>`);
            ml(false, 'requests');
            return;
        }
        //
        $.each(resp.Data, function (i, v) {
            ...
            //
            rows += `<tr data-id="${v.sid}" data-userid="${v.employee_sid}" data-name="${userRow.first_name} ${userRow.last_name}">`;
            rows += '    <td scope="row">';
            rows += '        <div class="employee-info">';
            rows += '            <figure>';
            rows += `                <img src="${getImageURL(userRow.image)}" class="img-circle emp-image" />`;
            rows += '            </figure>';
            rows += '            <div class="text">';
            rows += `                <h4>${userRow.first_name} ${userRow.last_name} </h4>`;
            rows += `                <p>${remakeEmployeeName(userRow, false)}</p>`;
            rows += `                <p><a href="${baseURL}employee_profile/${userRow.user_id}" target="_blank">Id: ${getEmployeeId(userRow.user_id, userRow.employee_number)}</a></p>`;
            rows += '            </div>';
            rows += '        </div>';
            rows += '    </td>';
            rows += `<td>
            ...
            <div class="progress" style="margin-top: 10px;">
                <div class="progress-bar progress-bar-success" role="progressbar" style="width: ${v.status == 'pending' ? (v.level_status != 'pending' ? 50 : 0) : 100}%;">
                    <span class="sr-only"> ${v.status == 'pending' ? (v.level_status != 'pending' ? 50 : 0) : 100} % Complete</span>
                </div>
                <p>${v.status == 'pending' ? (v.level_status != 'pending' ? 50 : 0) : 100}%</p>
            </div>
            ${getApproverLisiting(v.history)}
            </td>`;
            rows += `<td>
                        <p>${v.reason == '' || v.reason == null ? '-' : v.reason}</p>
                    </td>`;
            ...
            rows += `    <div class="dropdown" style="margin-top: 10px;">
                        ...
                        <li><a href="#" class="${v.archive == 1 ? 'jsActiveTimeOff' : 'jsArchiveTimeOff'}">${v.archive == 1 ? 'Activate' : 'Archive'}</a></li>
