$(function() {
    ...
    function setReportView() {
        ...
        //
        let
            individualEmployees = $('#jsIndividualEmployees').val() || [],
        ...
        employeeList.map(function(emp) {
            ...
            let job = emp['job_title'] != '' && emp['job_title'] != null ? emp['job_title'].replace(/[^a-zA-Z]/g, '').trim().toLowerCase() : '';
            //
            ...
            } else if (emp['registration_date'] != '' && emp['registration_date'] != null) {
                joinedAt = moment(emp['joined_at'], 'YYYY-MM-DD H:m:s');
            }
            //
            if (newHires == 30 && moment().diff(joinedAt, 'days') <= 30 ||
