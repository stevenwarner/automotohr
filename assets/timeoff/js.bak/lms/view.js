$(function() {
    ...
    $(".jsTeamShiftTab").click(function() {
        ...
        //
        callOBJ.Requests.Main.isMine = $(this).data("key") == "my" ? 1 : 0;
        window.timeoff.isMine = callOBJ.Requests.Main.isMine;
    ...
    function setTable(resp) {
        ...
        $.each(resp.Data, function(i, v) {
            ...
            //
            rows += `<tr data-id="${v.sid}" data-status="${v.status}" data-view="${expired}" data-userid="${v.employee_sid}" data-name="${userRow.first_name} ${userRow.last_name}">`;
            rows += '    <td scope="row">';
            rows += '        <div class="employee-info">';
            rows += "            <figure>";
            rows += `                <img src="${getImageURL(
        ...
        userRow.employee_number
          ...
          v.status == "cancelled" || expired == 1 ? "View" : "Edit"
            ...
            rows += ` <li><a href="${baseURL}timeoff/print/requests/${v.sid}" target="_blank">Print</a></li>`;
            rows += `<li><a href="${baseURL}timeoff/download/requests/${v.sid}" target="_blank">Download</a></li>`;
            rows += `            </ul>
                        </div>`;
            rows += `    </td>`;
            rows += `</tr>`;
