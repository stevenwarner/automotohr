$(function() {
    ...
    $(document).on('click', '.jsEditTimeOff', function(e) {
        ...
        let requestId = $(this).closest('tr').data('id'),
            employeeId = $(this).closest('tr').data('userid'),
            status = $(this).closest('tr').data('status'),
            view = $(this).closest('tr').data('view');
        ...
        Modal({
            Id: 'editModal',
            Title: `Edit Time off for ${$(this).closest('tr').data('name')}`,
            Body: '',
            Buttons: [
                `<a class="btn btn-success" target="_blank" style="margin-right: 5px; margin-top: -5px;" href="${baseURL}timeoff/print/requests/${requestId}"><i class="fa fa-print"></i> Print</a>`,
                `<a class="btn btn-success" target="_blank" style="margin-right: 5px; margin-top: -5px;" href="${baseURL}timeoff/download/requests/${requestId}"><i class="fa fa-download"></i> Download</a>`,
                status == 'cancelled' || view == 1 ? '' : '<button class="btn btn-success jsEditTimeOffBTN">Update</button>'
            ],
