			...
			(resp) => {
				...
				if (resp.Data.length > 0) {
					...
					resp.Data.map((policy) => {
						...
						// new policy holder
						if (newPolicyHolder[policy.Category] === undefined) {
							newPolicyHolder[policy.Category] = [];
						}
						newPolicyHolder[policy.Category].push(policy);
						//
					...
					for (let i in newPolicyHolder) {
						newRows += `<optgroup label="${i}">`;
						newPolicyHolder[i].map((policy) => {
							newRows += `
                            <option value="${policy.PolicyId}">${policy.Title}</option>
                            `;
						});
						newRows += `</optgroup>`;
					}
					$("#jsAddPolicy").html(newRows);
					$("#jsAddPolicy").select2();
