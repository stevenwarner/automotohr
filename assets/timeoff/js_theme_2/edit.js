$(function () {
	...
	$(document).on("click", ".jsEditTimeOffBTN", function (e) {
      ...
      */
		...
		let policy = getSelectedPolicy(selectedPolicyId);
	...
	$(document).on("click", ".jsEditTimeOff", function (e) {
		...
		Modal(
			...
			async () => {
				...
				cOBJ.startDate = moment(request.Data.request_from_date).format(
					"MM/DD/YYYY"
				);
				cOBJ.endDate = moment(request.Data.request_to_date).format(
					"MM/DD/YYYY"
				);
				...
				if ($("#jsCommentEdit").length > 0)
					CKEDITOR.replace("jsCommentEdit");
				//
				...
				cOBJ.dateRows.days.map((v) => {
					...
					let newDate = moment(v.date, "MM-DD-YYYY").format(
						"MMDDYYYY"
					);
					timeRowsOBJEdit[newDate] = {};
					timeRowsOBJEdit[newDate]["hour"] = newTime.hours;
					timeRowsOBJEdit[newDate]["partial"] =
						v.partial == undefined ? "fullday" : v.partial;
				...
				getSideBarPolicies(employeeId, cOBJ.policyId);
				//
				if (status == "cancelled" || view == 1) {
		...
		$("#editModal")
			...
			.text(
				$("#editModal")
					...
					.text()
					.trim()
					.replace("Edit Time off", "Balance History")
			);
		//
		$.post(handlerURL, {
	...
	$(document).on(
		"click",
		".jsCreateTimeOffBalanceBackEdit",
		function (event) {
			//
			event.preventDefault();
			//
			...
			$("#editModal")
				...
				.text(
					$("#editModal")
						...
						.text()
						.trim()
						.replace("Balance History", "Edit Time off")
				);
		}
	);
	...
	//
	function getSideBarPolicies(eId, pId) {
		//
			...
			(resp) => {
				...
				if (resp.Data.length > 0) {
					...
					resp.Data.map((policy) => {
						...
						//
						if (newPoliciesObj[policy["Category"]] === undefined)
							newPoliciesObj[policy["Category"]] = [];
                        ...
                        Last Anniversary: ${moment(
							policy.lastanniversary,
							"YYYY/MM/DD"
						).format(timeoffDateFormat)} <br>
                        Upcoming Anniversary: ${moment(
							policy.upcominganniversary,
							"YYYY/MM/DD"
						).format(timeoffDateFormat)}
                        </span> 
                        </div>
				...
				if (pId !== undefined) {
					$("#jsEditPolicy").select2("val", pId);
				}
				//
				if (window.timeoff.cPolicies.length == 0) {
					$("#jsAsOfTodayPolicies").html(
						'<div class="alert alert-success">No policies found.</div>'
					);
	...
	//
	function setApproversView(approvers, history) {
	...
	//
	function setHistoryView(history) {
		//
		let rows = "";
		//
		if (history.length == 0) {
			...
			history.map((v) => {
				...
				rows += `<p>${act}</p>`;
				rows += `<p>Action Taken: ${moment(v.created_at).format(
					timeoffDateFormatWithTime
				)}</p>`;
				rows += `<p>${comment}</p><hr />`;
			});
	...
	//
	function getApproverAction(approverId, history) {
		//
		...
		history.map((v) => {
			...
			if (action.status == "approved") {
				row[0] = ` has approved the time-off at ${moment(
					v.created_at
				).format(timeoffDateFormatWithTime)}`;
				row[1] = `approved`;
			} else if (action.status == "rejected") {
				row[0] = ` has rejected the time-off at ${moment(
					v.created_at
				).format(timeoffDateFormatWithTime)}`;
				row[1] = `rejected`;
			}
	...
	$(document).on("change", "#jsEditPolicy", function () {
		...
		//
		var singlePolicy = getPolicy($(this).val(), window.timeoff.cPolicies);
		//
