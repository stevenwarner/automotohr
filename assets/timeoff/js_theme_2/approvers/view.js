$(function() {
    ...
    $(document).on('click', '.js-archive-btn', function(e) {
        ...
        alertify.confirm('Do you really want to deactivate this approver?', function() {
            ...
            post.public = 0;
            post.approverId = _this.closest('.jsBox').data('id');
            //
            ml(true, 'approver');
            //
    ...
    $(document).on('click', '.jsApproverHistory', function() {
        Modal({
            Id: 1,
            Title: `Approver History for ${$(this).closest('.jsBox').data('name')}`,
            Body: getHistoryTemplate(),
            Loader: 'jsApproverHistoryLoader'
        }, () => {
            // Fetch history
            fetchApproverHistory($(this).closest('.jsBox').data('id'));
        });
    });
    ...
    //
    function getApproverBox(v, userRow) {
            ...
            <div class="csBox csShadow csRadius5 mt10 jsBox" data-id="${v.approver_id}" data-name="${v.first_name} ${v.last_name}">
                ...
                <div class="csBoxContent">
                    ...
                    <div class="csBoxBalanceSection">
                        <div class="col-sm-12">
                            <p class="${v.approver_percentage == 1 ? 'csTxtSuccess' : 'csTxtDanger' }"><strong>${v.approver_percentage == 1 ? "Yes" : "No"}</strong></p>
                            <p>Can Approve</p>
                        </div>
                        <div class="clearfix"></div>
                    </div>
                    ...
                    <div class="csBoxBalanceSection">
                        <div class="col-sm-12">
                            <p><strong>${
                                v.is_department == 1 ? 
                                (
                                    v.department_name == null ? 'All Departments' : ucwords(v.department_name)
                                ) :
                                (
