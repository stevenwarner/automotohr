$(function() {
     ...
     * @param Object event
     */
    $(document).on('click', '.JsSendReminderEmailModalSaveBtn', function(event) {
        ...
        if (obj.type.length == 0) {
            alertify.alert('Warning!', 'Please, select at least one document type.', function() {});
            return;
        }
        //
        ...
        if (obj.userType == 'applicant') {
            alertify.confirm('This action will assign the applicant the selected document in order to send a reminder email. <br /> Do you want to continue?', function() {
    ...
    function loadReminderModal(type) {
        ...
        html += '                <h4 class="modal-title"><strong>Send An Email Reminder</strong></h4>';
        html += '            </div>';
        html += '            <div class="modal-body" style="min-height: 200px;">';
        html += '                <div class="csCommonLoader JsSendReminderEmailModalLoader"><i class="fa fa-circle-o-notch fa-spin" aria-hidden="true"></i></div>';
        html += '               <div id="JsSendReminderEmailModalBody"></div>';
        html += '            </div>';
        html += '            <div class="modal-footer">';
        html += '                <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>';
        html += '                <button type="button" class="btn btn-success JsSendReminderEmailModalSaveBtn dn">Send Reminder</button>';
        html += '            </div>';
        html += '        </div>';
        html += '    </div>';
        html += '</div>';
        ...
        //
        var options = [
            { id: "emergency-contact", text: '', html: "<strong>Emergency Contacts </strong>" + getLastSent("emergency-contact") + "" },
            { id: "occupational-license", text: '', html: "<strong>Occupational License </strong>" + getLastSent("occupational-license") + "" },
            { id: "drivers-license", text: '', html: "<strong>Drivers License </strong>" + getLastSent("drivers-license") + "" },
            { id: "dependents", text: '', html: "<strong>Dependents </strong>" + getLastSent("dependents") + "" },
            { id: "direct-deposit-information", text: '', html: "<strong>Direct Deposit Information</strong>" + getLastSent("direct-deposit-information") + "" }
        ];
        //
        var selectedOptions = [];
        //
        ...
        //
        $.get(window.sre.url + 'get_send_reminder_email_body')
            .done(function(resp) {
    ...
    function loadReminderHistoryModal() {
        ...
        html += '            <div class="modal-header">';
        html += '                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>';
        html += '                <h4 class="modal-title"><strong>Email Reminder History</strong></h4>';
        html += '            </div>';
        html += '            <div class="modal-body" style="min-height: 200px;">';
        ...
        if (logs.length) {
            logs.map(function(log) {
                ...
                html += '                               <a data-toggle="collapse" class="jsHistoryToggle" href="#' + (key) + '"><span class="pull-right"><i class="fa fa-plus-circle" aria-hidden="true"></i></span><strong>' + (slugToName(log['module_type'])) + '</strong> <br /> The last email reminder was sent on at ' + (moment(log['updated_at'], 'YYYY-MM-DD H:m:s').format('MMM DD YYYY, ddd hh:mm a')) + ' by ' + (log['name']) + ' </a>';
                html += '                           </h4>';
                html += '                       </div>';
                html += '                       <div id="' + (key) + '" class="panel-collapse collapse">';
                ...
                html += '                                       <thead>';
                html += '                                           <tr>';
                html += '                                               <th scope="col">Employee</th>';
                html += '                                               <th scope="col">Note</th>';
                html += '                                               <th scope="col">Action Date</th>';
                html += '                                           </tr>';
                html += '                                       </thead>';
                html += '                                       <tbody>';
                if (log['logs'].length === 0) {
                    html += '                                           <tr>';
                    html += '                                               <td>';
                    html += '                                                   <p class="text-center alert alert-info"><strong>The log is empty.</strong></p>';
                    html += '                                               </td>';
                    html += '                                           </tr>';
                } else {
                    //
                    log['logs'].map(function(emp) {
                        html += '                                           <tr>';
                        html += '                                               <td><strong>' + (emp['name']) + '</strong> <br /> ' + (emp['role']) + '</td>';
                        html += '                                               <td>' + (emp['note'] == '' || emp['note'] == null ? '-' : emp['note']) + '</td>';
                        html += '                                               <td>' + (moment(emp['created_at'], 'YYYY-MM-DD H:m:s').format('MMM DD YYYY, ddd hh:mm a')) + '</td>';
                        html += '                                           </tr>';
                        html += '                                       </tbody>';
                    });
                }
            ...
            html += '<p class="text-center alert alert-info"><strong>No history found.</strong></p>';
        }
    ...
    function getUserSendReminderEmailHistory() {
        $.get(window.sre.url + 'get_send_reminder_email_history/' + window.sre.userId + '/' + window.sre.userType)
            .done(function(resp) {
                //
                if (resp.length === 0) {
                    return;
    ...
    function getLastSent(slug) {
        return lastLogs[slug] !== undefined ? ' ( The last email reminder was sent on at ' + (moment(lastLogs[slug]['sent_at']).format('MM/DD/YY H:mm A')) + ' by ' + (lastLogs[slug]['sent_by']) + ' )' : ''
    }
    ...
    function sendReminderEmail(obj) {
        ...
        //
        $.post(
            window.sre.url + 'send_reminder_email_by_type',
            obj
        ).done(function(resp) {
            //
            if (resp == 'success') {
                ...
                $('#JsSendReminderEmailModal').remove();
                //
                alertify.alert('Success!', 'You have successfully sent a reminder email.', function() {
