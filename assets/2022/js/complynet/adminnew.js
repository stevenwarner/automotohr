$(function ComplyNetManagement() {
    ...
    $('.jsSaveLinkLocationNew').on('click', function () {
        var automotocompanySid = $('#automotoHRCompany').val();
        var complyNetLocationId = $('#complyNetLocation').val();
        var complyNetLocationName = $('#complyNetLocation option:selected').text();
        var automotocompanyName = $('#automotoHRCompany option:selected').text();

        if (automotocompanySid == "0" || automotocompanySid == undefined) {
            alertify.alert("Notice", "Please select AutomotoHR Company to link");
            return;
        }
        //
        if (complyNetLocationId == 0 || complyNetLocationId == undefined) {
        ...
        $.ajax({
            ...
            data: {
                ...
                automotocompany_Name: automotocompanyName
